...
const COLUMNS = [
  ...
  {
    key: 'linkedin_status_rejah',
    label: 'LinkedIn (Rejah)',
    width: 190,
    type: 'select',
    options: CONNECTION_STATUSES,
  },
  {
    key: 'linkedin_status_rahul',
    label: 'LinkedIn (Rahul)',
    width: 190,
    type: 'select',
    options: CONNECTION_STATUSES,
  },
...
function EditableCell({ lead, col, onSave, isEditing, onStartEdit, onStopEdit }) {
  ...
  useEffect(() => {
    setVal(getCellValue(lead, col.key))
  }, [lead[col.key]])
  useEffect(() => {
  ...
  if (col.type === 'date')
    return React.createElement('div', { style: cell.readonly }, getCellValue(lead, col.key))
  ...
  if (col.type === 'url') {
    ...
    if (!displayVal)
      return React.createElement('div', { style: cell.empty, onClick: onStartEdit }, '—')
    const shortUrl = displayVal
      .replace('https://www.linkedin.com/in/', '')
      .replace('https://', '')
      .split('/')[0]
    return React.createElement(
      'a',
      { href: displayVal, style: cell.link, onClick: (e) => e.stopPropagation() },
      shortUrl
    )
  }
  ...
  if (col.type === 'select') {
      ...
      return React.createElement(
        'select',
        {
          ...
          onChange: (e) => {
            ...
            onSave(lead.id, col.key, e.target.value)
            onStopEdit()
          },
          onBlur: onStopEdit,
          style: cell.select,
    ...
    return React.createElement(
      'div',
      { style: { ...cell.text, color, cursor: 'pointer' }, onClick: onStartEdit },
      displayVal || '—'
    )
  }
  ...
  if (col.key === 'email' && displayVal) {
    ...
    return React.createElement(
      'div',
      {
        style: { ...cell.text, cursor: 'text', display: 'flex', alignItems: 'center', gap: 5 },
        onClick: onStartEdit,
      },
      displayVal,
      es &&
  ...
  return React.createElement(
    'div',
    { style: { ...cell.text, cursor: 'text' }, onClick: onStartEdit },
...
export default function SpreadsheetView({ leads, onClose, onLeadUpdate, onRefresh }) {
  ...
  const accountManager =
    localStorage.getItem(user?.id ? `fullName_${user.id}` : 'fullName') ||
  ...
  const isDecisionMakerSS = (lead) =>
    DM_TITLES_SS.some((t) => (lead.title || '').toLowerCase().includes(t))
  const isSecuritySS = (lead) =>
    [
      'ciso',
      'security engineer',
      'security analyst',
      ...
      'security',
    ].some((t) => (lead.title || '').toLowerCase().includes(t))
  ...
  async function handleAutofill() {
    ...
    while (hasMore) {
      try {
        ...
        setAutofillMsg(
          'Processing ' + Math.min(batchStart, toFill.length) + '/' + toFill.length + '...'
        )
    ...
    if (Object.keys(allUpdates).length > 0) {
      setLocalLeads((prev) => [
        ...prev.map((l) => (allUpdates[l.id] ? { ...l, ...allUpdates[l.id] } : l)),
      ])
      setTimeout(() => {
  ...
  function handleExport() {
    ...
    const rows = toExport.map((lead) =>
      columns.filter((c) => c.key !== '_serial').map((c) => getCellValue(lead, c.key))
    )
      ...
      React.createElement(
        ...
        React.createElement(
            ...
            React.createElement(
              'span',
              {
                style: {
                  ...
                  color: autofillMsg.startsWith('Done')
                    ? '#4a7c59'
                    : autofillMsg.startsWith('Refresh')
          ...
          React.createElement(
            ...
            companies.map((c) =>
              React.createElement('option', { key: c, value: c }, c === 'all' ? 'All companies' : c)
            )
          ),
          ...
          React.createElement(
            'button',
            { style: hdr.exportBtn, onClick: handleExport },
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              React.createElement(
                'th',
                { style: { ...th, width: '44px', position: 'sticky', top: 0, zIndex: 10 } },
                React.createElement('input', {
                  type: 'checkbox',
          ...
          React.createElement(
            ...
            filtered.map((lead, rowIndex) =>
              React.createElement(
                ...
                React.createElement(
                  'td',
                  { style: { ...td, width: '44px', textAlign: 'center' } },
                  React.createElement('input', {
                    type: 'checkbox',
...
const hdr = {
  bar: {
    ...
    alignItems: 'center',
    padding: '14px 24px',
    borderBottom: '1px solid rgba(253,253,253,0.08)',
    background: 'var(--text)',
    flexShrink: 0,
    gap: '10px',
    flexWrap: 'wrap',
