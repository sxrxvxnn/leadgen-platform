...
export default function AddCompanyModal({ onClose, onRefresh, onAdded }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const newCompany = created?.data?.company
      setStep('saved')
      // Optimistic insert: show immediately without waiting for a full list re-fetch
      if (newCompany && onAdded) {
        onAdded(newCompany)
      } else {
        onRefresh?.()
      }
      ...
      const msg =
        typeof raw === 'string' && raw.includes("'message'")
      ...
      <div style={s.modal} onClick={(e) => e.stopPropagation()}>
        ...
        {(step === 'input' || step === 'error') && (
          <div style={s.body}>
            ...
            <div style={s.fieldRow}>
              <label style={s.label}>
                Website <span style={s.optional}>— auto-filled when you pick a suggestion</span>
              </label>
              <input
                type="text"
            ...
            <p style={s.hint}>
              Type a company name → pick from suggestions → hit Find & Fill to enrich with LinkedIn
              data.
            </p>
            <button
              onClick={handleFind}
        ...
        {step === 'loading' && (
          <div style={{ ...s.body, textAlign: 'center', padding: '48px 24px' }}>
            ...
            >
              Searching website → extracting LinkedIn → scraping public data
            </p>
          </div>
        )}
        ...
        {step === 'form' && (
          <div style={s.body}>
            {foundFields.length > 0 && (
              <div style={s.banner}>
                ✓ {foundFields.length} field{foundFields.length > 1 ? 's' : ''} auto-filled from
                LinkedIn — review before saving
              </div>
            )}
            ...
            {linkedinPeopleUrl && foundFields.length < 3 && (
              <div style={s.extensionNote}>
                ...
                >
                  For associated members, visit the People tab with the extension:
                </p>
                <a
                  href={linkedinPeopleUrl}
                  target="_blank"
                  rel="noreferrer"
                  style={s.peopleLinkBtn}
                >
                  Open People Tab →
                </a>
              </div>
            ...
            <div style={s.actions}>
              <button onClick={handleReset} style={s.secondaryBtn}>
                ← Back
              </button>
              <button
                onClick={handleSave}
...
const s = {
  ...
  dropdown: {
    ...
    border: '1px solid var(--border)',
    borderRadius: '8px',
    boxShadow: '0 8px 24px rgba(29,27,27,0.12)',
    zIndex: 100,
    overflow: 'hidden',
  },
  suggItem: {
