...
export default function CanvasText({
  ...
  useEffect(() => {
    ...
    const lines = Array.from({ length: 18 }, (_, i) => ({
      ...
      y0: -height * 0.5 + Math.random() * height * 2,
      x1: Math.random() * width,
      y1: -height * 0.5 + Math.random() * height * 2,
      // control points
      cx0: Math.random() * width,
      cy0: -height + Math.random() * height * 2,
      cx1: Math.random() * width,
      cy1: -height + Math.random() * height * 2,
    }))
    ...
    function draw() {
      ...
      lines.forEach((l) => {
        ...
        ctx.moveTo(l.x0 + Math.sin(p) * 55, l.y0 + Math.cos(p * 0.8) * 28)
        ctx.bezierCurveTo(
  ...
  return (
    <canvas ref={canvasRef} width={width} height={height} style={{ display: 'block', ...style }} />
  )
}
