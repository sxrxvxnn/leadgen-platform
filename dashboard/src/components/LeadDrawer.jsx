...
const SECURITY_RE =
  /\b(ciso|chief information security|cybersecurity|infosec|information security officer|grc|vapt|penetration tester|vulnerability|security analyst|security engineer|security architect|security manager|security director|soc analyst|devsecops|cloud security)\b/i
...
function scoreColor(s) {
  ...
  if (s >= 40)
    return { text: '#a86448', bg: 'rgba(168,100,72,0.10)', border: 'rgba(168,100,72,0.3)' }
  ...
  return (
    <button
      ...
      style={{
        ...
        borderRadius: 4,
        color: copied ? '#4a7c59' : 'var(--text-muted)',
        cursor: 'pointer',
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
        ...
        <button
          ...
          style={{
            ...
            fontWeight: 600,
            padding: '7px 12px',
            background: saved ? 'rgba(74,124,89,0.15)' : 'var(--surface)',
            border: `1px solid ${saved ? 'rgba(74,124,89,0.4)' : 'var(--border)'}`,
            borderRadius: 5,
            color: saved ? '#4a7c59' : 'var(--text-muted)',
...
function lsGetU(user, key, fallback = '') {
  return (
    localStorage.getItem(user?.id ? `${key}_${user.id}` : key) ??
...
export default function LeadDrawer({ lead, onClose, onUpdate, onDelete }) {
  ...
  const [verifyResult, setVerifyResult] = useState(
    lead?.email_status ? { status: lead.email_status, score: lead.email_score } : null
  )
  ...
  async function handleScore() {
    ...
    try {
      ...
      setLocalLead((prev) => ({
        ...prev,
        icp_score: r.data.score,
        icp_score_reason: r.data.reason,
      }))
      ...
      >
            ...
            <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
              <button
                ...
                style={{
                  ...
                  cursor: 'pointer',
                  fontSize: 18,
                  color: L.starred ? 'var(--accent)' : 'var(--border-strong)',
                  padding: '2px 4px',
                }}
              >
                {L.starred ? '★' : '☆'}
        ...
        <div
          style={{
            padding: '20px 28px',
            borderBottom: '1px solid var(--border)',
            background: sc != null ? colors.bg : 'transparent',
          }}
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <div style={{ flex: 1 }}>
              {scReason ? (
                ...
                >
                  Not scored yet. Click Score to evaluate this lead against your ICP criteria.
                </p>
              )}
              <button
                onClick={handleScore}
                ...
                style={{
                  ...
                  letterSpacing: '0.06em',
                  padding: '5px 12px',
                  background: scoring ? 'var(--surface)' : 'var(--text)',
                  color: scoring ? 'var(--text-muted)' : 'var(--bg)',
                  border: 'none',
                  borderRadius: 4,
        ...
        >
          ...
          <div>
              ...
              <div style={{ marginBottom: 10 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <p style={{ fontFamily: MONO, fontSize: 13, color: 'var(--text)', margin: 0 }}>
                      {L.email}
                    </p>
                  </div>
                ...
                {verifyResult &&
                  (() => {
                    ...
                    const label =
                      s === 'valid'
                        ? `Valid · ${verifyResult.score || 0}% deliverability`
                        : s === 'risky'
                          ? `Risky · ${verifyResult.score || 0}% — send with caution`
                          : s === 'invalid'
                            ? 'Invalid — likely to bounce'
                            : verifyResult.error || s
                    return (
                      <p
                        style={{
                          fontFamily: MONO,
            ...
            {!L.email && (
              ...
              >
                No email — use Email button in the table to find one
              </p>
            )}
            {(L.phone || L.location) && (
              <div
                ...
                <div style={{ padding: '8px 12px', background: 'var(--bg)' }}>
                  ...
                  <p
                    style={{
                      fontFamily: MONO,
                      fontSize: 11,
                      color: L.location ? 'var(--text)' : 'var(--text-muted)',
                      margin: 0,
                    }}
                  >
                    {L.location || '—'}
            ...
            {(L.profile_url || L.linkedin_url) && (
              <div style={{ marginBottom: 12 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                  <a
                    href={L.profile_url || L.linkedin_url}
                    target="_blank"
                  ...
                  <button
                    ...
                    style={{
                      ...
                      padding: '4px 10px',
                      background: liEnrichResult?.ok ? '#4a7c59' : 'var(--surface)',
                      color: liEnrichResult?.ok ? '#fff' : 'var(--text-secondary)',
                      border: '1px solid var(--border)',
                      borderRadius: 4,
                      cursor: enrichingLI ? 'default' : 'pointer',
                    }}
                  >
                    {enrichingLI
                      ? 'Enriching…'
                      : liEnrichResult?.ok
                        ? liEnrichResult.fields.length
                          ? `+${liEnrichResult.fields.length} fields`
                          : 'Up to date'
                        : 'Enrich from LinkedIn'}
                  </button>
                </div>
                {liEnrichResult?.error && (
                  <p style={{ fontFamily: MONO, fontSize: 9, color: '#E7000B', margin: '4px 0 0' }}>
                    {liEnrichResult.error}
                  </p>
                )}
              </div>
          ...
          <div>
            ...
            >
              Generate a hyper-personalized cold email in 2 seconds based on this lead's profile and
              company context.
            </p>
            <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}>
              <button
                ...
                style={{
                  ...
                  letterSpacing: '0.07em',
                  padding: '8px 16px',
                  background: drafting ? 'var(--surface)' : '#4a7c59',
                  color: drafting ? 'var(--text-muted)' : '#fff',
                  border: 'none',
                  borderRadius: 5,
              ...
              <button
                ...
                style={{
                  ...
                  letterSpacing: '0.07em',
                  padding: '8px 14px',
                  background: iceLoading ? 'var(--surface)' : 'rgba(124,58,237,0.12)',
                  color: iceLoading ? 'var(--text-muted)' : '#7c3aed',
                  border: '1px solid rgba(124,58,237,0.3)',
                  borderRadius: 5,
                  cursor: iceLoading ? 'default' : 'pointer',
                  transition: 'all 0.15s',
                }}
              >
            ...
            {showTplPicker && (
              ...
              >
                {templates && templates.length === 0 && (
                  ...
                  >
                    No saved templates yet. Write an email and save it.
                  </p>
                )}
                {templates &&
                  templates.map((tpl) => (
            ...
            {draft && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                >
                  ...
                  <p style={{ fontFamily: MONO, fontSize: 12, color: 'var(--text)', margin: 0 }}>
                    {draft.subject}
                  </p>
                </div>
                <div
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  <button
                    ...
                    style={{
                      ...
                      letterSpacing: '0.07em',
                      padding: '8px',
                      background: tracking ? 'var(--surface)' : 'var(--text)',
                      color: tracking ? 'var(--text-muted)' : 'var(--bg)',
                      border: 'none',
                      borderRadius: 5,
                  ...
                  <button
                    ...
                    style={{
                      ...
                      letterSpacing: '0.07em',
                      padding: '8px 14px',
                      background: sendResult?.ok ? '#4a7c59' : 'var(--accent)',
                      color: '#fff',
                      border: 'none',
                      borderRadius: 5,
                      cursor: sending || !L.email ? 'default' : 'pointer',
                      opacity: !L.email ? 0.4 : 1,
                    }}
                  >
                ...
                <button
                  ...
                  style={{
                    ...
                    border: '1px solid var(--border)',
                    borderRadius: 4,
                    color: tplSaved ? '#4a7c59' : 'var(--text-muted)',
                    cursor: savingTpl ? 'default' : 'pointer',
