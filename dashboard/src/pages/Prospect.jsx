...
function TagInput({ value, onChange, placeholder, suggestions = [], pendingRef }) {
  ...
  const filtered = suggestions
    .filter((s) => input && s.toLowerCase().includes(input.toLowerCase()) && !value.includes(s))
    .slice(0, 6)
...
function PersonRow({
  ...
  const workEmail =
    enrichedEmails[person.id] || person.work_email || (person.emails || [])[0]?.address || null

  async function revealEmail() {
    if (workEmail) {
      setRevealedEmail(workEmail)
      return
    }
    setRevealing(true)
    try {
      const res = await api.post('/prospect/reveal-email', {
  ...
  if (isVerifying) {
    ...
    btnColor = '#60a5fa'
    btnBorder = '1px solid rgba(59,130,246,0.25)'
  } else if (isAdded) {
    btnLabel = 'Added ✓'
    btnBg = 'rgba(5,150,105,0.08)'
    btnColor = '#34d399'
    btnBorder = '1px solid rgba(5,150,105,0.25)'
  }
  ...
  return (
    <div
      style={{
        display: 'grid',
        gridTemplateColumns: '32px 1fr 160px 190px 160px',
        gap: 12,
        alignItems: 'center',
        padding: '12px 20px',
        borderBottom: '1px solid var(--border)',
        background: isSelected ? 'rgba(231,0,11,0.04)' : 'transparent',
        ...
        {displayEmail ? (
          <p
            style={{
              ...
              fontSize: 11,
              color:
                displayEmail === '—' || displayEmail === 'Not found'
                  ? 'var(--text-muted)'
                  : 'var(--text)',
              margin: 0,
              overflow: 'hidden',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <button
          ...
          style={{
            ...
            border: btnBorder,
            borderRadius: 5,
            cursor: isAdded || isVerifying || isDup ? 'default' : 'pointer',
            whiteSpace: 'nowrap',
          }}
        >
          {btnLabel}
...
export default function Prospect() {
  ...
  useEffect(() => {
    ...
    api
      .get('/saved-searches')
      .then((r) => setSavedSearches(r.data?.searches || []))
      .catch(() => {})
  }, [])
  ...
  const [searchHistory, setSearchHistory] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('prospect_search_history') || '[]')
    } catch {
      return []
    }
  })
  ...
  function buildFilters() {
    ...
    const activeTitles = [
      ...titles,
      ...(pendingTitle.current.trim() ? [pendingTitle.current.trim()] : []),
    ]
    const activeLocs = [
      ...locations,
      ...(pendingLocation.current.trim() ? [pendingLocation.current.trim()] : []),
    ]
  ...
  async function handleAddSelected() {
    const toAdd = results.filter(
      (p) => selected.has(p.id) && !addedIds.has(p.id) && !verifyingIds.has(p.id)
    )
  ...
  async function handleEnrollSelected(seqId) {
    ...
    try {
      ...
      if (leadIds.length) {
        ...
        setEnrollResult(
          `Enrolled ${res.data.enrolled} leads.${res.data.skipped ? ` ${res.data.skipped} skipped.` : ''}`
        )
      }
      invalidateCache('/leads')
    } catch (e) {
  ...
  return (
    <div style={{ padding: '36px 40px', maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div style={{ position: 'relative', marginLeft: savedSearches.length ? 8 : 0 }}>
            <button
              ...
              style={{
                ...
                fontSize: 10,
                padding: '5px 12px',
                background: showHistory ? 'var(--surface)' : 'none',
                border: '1px solid var(--border)',
                borderRadius: 20,
                color: 'var(--text-muted)',
                cursor: 'pointer',
      ...
      >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
          ...
          <div>
            ...
            <TagInput
              ...
              placeholder="Add country or city… (press Enter)"
              pendingRef={pendingLocation}
            />
          </div>
        </div>
            ...
            {SIZE_OPTIONS.map((s) => (
              <button
                ...
                style={{
                  ...
                  background: sizes.includes(s.value) ? 'var(--text)' : 'var(--bg)',
                  color: sizes.includes(s.value) ? 'var(--bg)' : 'var(--text-muted)',
                  border: sizes.includes(s.value)
                    ? '1px solid var(--text)'
        ...
        >
          {error && (
            <p style={{ fontFamily: MONO, fontSize: 11, color: '#E7000B', margin: 0 }}>{error}</p>
          )}
          <div style={{ flex: 1 }} />
          {/* Save search */}
          {showSaveInput ? (
        ...
        >
          ...
          >
            ...
            {enrollResult ? (
              ...
              >
                <p style={{ fontFamily: MONO, fontSize: 12, color: '#4a7c59', margin: 0 }}>
                  {enrollResult}
                </p>
              </div>
            ) : sequences.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {sequences.map((seq) => (
                  <button
                    key={seq.id}
                    onClick={() => handleEnrollSelected(seq.id)}
                    disabled={enrolling}
                    style={{
                      padding: '12px 16px',
                      background: 'var(--surface)',
                  ...
                  >
                    ...
                    >
                      {seq.steps?.length || 0} steps · {seq.enrolled_count || 0} enrolled
                    </div>
                  </button>
                ))}
                {enrolling && (
      ...
      {!searched && !loading && (
        <div style={{ textAlign: 'center', padding: '80px 20px' }}>
          ...
          <p style={{ fontFamily: MONO, fontSize: 12, color: 'var(--text-muted)', margin: 0 }}>
            Fill in any filter and hit Search. Leads are live-verified on LinkedIn before saving.
          </p>
        </div>
      )}
    </div>
