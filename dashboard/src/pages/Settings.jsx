...
function uk(user, key) {
  return user?.id ? `${key}_${user.id}` : key
}
function lsGet(user, key, fallback = '') {
  return localStorage.getItem(uk(user, key)) ?? localStorage.getItem(key) ?? fallback
}
function lsSet(user, key, value) {
  localStorage.setItem(uk(user, key), value)
...
export default function Settings() {
  ...
  async function handleDeleteAutoSmtp() {
    if (
      !confirm(
        'Remove saved SMTP credentials from server? Sequence automation will stop sending emails until reconfigured.'
      )
    )
      return
  ...
  useEffect(() => {
    getWorkerHealth()
      .then((r) => setWorkerHealth(r.data))
      .catch(() => {})
    getCronStatus()
      .then((r) => setCronRows(r.data || []))
      .catch(() => {})
  }, [])
  ...
  async function handleSaveWarmup() {
    ...
    try {
      ...
      setWarmup({
        ...
        config: cfg,
        day: warmup?.day || 0,
        daily_target: warmup?.daily_target || 2,
      })
      setWarmupSaved(true)
  ...
  async function testWebhook(id) {
    ...
    try {
      ...
      setWhTestResult((t) => ({
        ...t,
        [id]: r.data.ok ? 'ok' : `fail (${r.data.status_code || r.data.error})`,
      }))
    } catch {
  ...
  async function handleSaveCal() {
    ...
    if (calLink && !calLink.startsWith('http')) {
      setCalError('Enter a valid URL starting with https://')
      return
    }
    setCalSaving(true)
    try {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      {/* Two-column layout */}
      <motion.div
        style={{ display: 'flex', maxWidth: '1100px', margin: '0 auto', padding: '0 48px 80px' }}
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
              ...
              >
                ...
                <span
                  style={{
                    fontFamily: 'var(--font-mono)',
                    fontSize: '11px',
                    color: activeSection === sec.id ? 'var(--text)' : 'var(--text-secondary)',
        ...
        <main style={{ flex: 1, paddingTop: '48px', minWidth: 0 }}>
          ...
          <section
            ...
            style={{ ...s.section, borderBottom: '1px solid var(--border)' }}
          >
            <SectionHeader num="02" title="Email Sending" />
            <p style={s.sectionHint}>
              Connect any email account via SMTP to send emails directly from Sonar — no
              copy-pasting required. Gmail users: use an App Password (Google Account → Security →
              2FA → App Passwords).
            </p>
            ...
            >
              ...
              >
                <strong style={{ color: 'var(--text)' }}>Gmail:</strong> smtp.gmail.com · port 587 ·
                your Gmail address ·{' '}
                <a
                ...
                <br />
                <strong style={{ color: 'var(--text)' }}>Outlook:</strong> smtp.office365.com · port
                587 · your email · account password
                <br />
          ...
          >
            ...
            <p style={s.sectionHint}>
              Save SMTP credentials server-side so sequence steps run automatically on schedule —
              even when you're offline. These are stored encrypted on our server and used only by
              the sequence cron job. Sequences run every 6 hours.
            </p>
            ...
            {autoConfigured && (
              ...
              >
                ...
                >
                  Sequence emails will send automatically via{' '}
                  {autoSmtp.smtp_user || 'saved account'}.
                </span>
                <button
                  onClick={handleDeleteAutoSmtp}
            ...
            >
              <Field label="From name" hint="Shown as sender name in sequence emails">
                <input
                  value={autoSmtp.from_name}
                  onChange={(e) => setAutoSmtp((p) => ({ ...p, from_name: e.target.value }))}
                  placeholder="Jane Smith"
                  style={s.input}
                />
              </Field>
              <Field label="From email" hint="Reply-to address">
                <input
                  value={autoSmtp.from_email}
                  onChange={(e) => setAutoSmtp((p) => ({ ...p, from_email: e.target.value }))}
                  placeholder="jane@company.com"
                  style={s.input}
                />
              </Field>
            ...
            >
              <Field label="SMTP host">
                <input
                  value={autoSmtp.smtp_host}
                  onChange={(e) => setAutoSmtp((p) => ({ ...p, smtp_host: e.target.value }))}
                  placeholder="smtp.gmail.com"
                  style={s.input}
                />
              </Field>
              <Field label="Port">
                <input
                  value={autoSmtp.smtp_port}
                  onChange={(e) => setAutoSmtp((p) => ({ ...p, smtp_port: e.target.value }))}
                  placeholder="587"
                  style={s.input}
                />
              </Field>
            ...
            >
              <Field label="SMTP username">
                <input
                  value={autoSmtp.smtp_user}
                  onChange={(e) => setAutoSmtp((p) => ({ ...p, smtp_user: e.target.value }))}
                  placeholder="jane@gmail.com"
                  style={s.input}
                />
              </Field>
              <Field label="Password / App Password">
                <KeyInput
                  value={autoSmtp.smtp_pass}
                  onChange={(v) => setAutoSmtp((p) => ({ ...p, smtp_pass: v }))}
                  placeholder="App password or SMTP password"
                  set={autoConfigured && autoSmtp.smtp_pass === '••••••••'}
                />
              </Field>
            </div>
            ...
            <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
              <button
                onClick={handleSaveAutoSmtp}
                disabled={autoSaving || !autoSmtp.smtp_user}
                style={{
                  fontFamily: 'var(--font-mono)',
                  fontSize: 11,
                  fontWeight: 600,
                  padding: '10px 20px',
                  background: autoSaved ? '#4a7c59' : 'var(--accent)',
                  color: '#fff',
                  border: 'none',
                  borderRadius: 3,
                  cursor: autoSaving || !autoSmtp.smtp_user ? 'default' : 'pointer',
                  opacity: !autoSmtp.smtp_user ? 0.5 : 1,
              ...
              <span
                style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--text-muted)' }}
              >
                Tokens: {'{{name}}'} {'{{first_name}}'} {'{{company}}'} {'{{title}}'}{' '}
                {'{{cal_link}}'}
              </span>
            </div>
            ...
            <div style={{ marginTop: 32, paddingTop: 24, borderTop: '1px solid var(--border)' }}>
              ...
              >
                Check SPF and DMARC records for your sending domain. Missing records increase spam
                placement.
              </p>
              <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                <input
                  value={
                    domainInput ||
                    (autoSmtp.from_email.includes('@') ? autoSmtp.from_email.split('@')[1] : '')
                  }
                ...
                <button
                  ...
                  style={{
                    ...
                    border: '1px solid var(--border)',
                    borderRadius: 3,
                    cursor: domainChecking ? 'default' : 'pointer',
                    letterSpacing: '0.04em',
                    whiteSpace: 'nowrap',
                  }}
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                    <div
                      style={{
                        ...
                        borderRadius: '50%',
                        background:
                          domainHealth.score >= 80
                            ? 'rgba(74,124,89,0.15)'
                            : domainHealth.score >= 40
                              ? 'rgba(234,179,8,0.12)'
                              : 'rgba(239,68,68,0.12)',
                        display: 'flex',
                        alignItems: 'center',
                    ...
                    <div style={{ marginLeft: 'auto', display: 'flex', gap: 6 }}>
                      <span
                        style={{
                          ...
                          borderRadius: 3,
                          background:
                            domainHealth.spf.ok && !domainHealth.spf.warn
                              ? 'rgba(74,124,89,0.12)'
                              : 'rgba(239,68,68,0.1)',
                          color:
                            domainHealth.spf.ok && !domainHealth.spf.warn ? '#4a7c59' : '#ef4444',
                        }}
                      >
                      ...
                      <span
                        style={{
                          ...
                          borderRadius: 3,
                          background:
                            domainHealth.dmarc.ok && !domainHealth.dmarc.warn
                              ? 'rgba(74,124,89,0.12)'
                              : 'rgba(239,68,68,0.1)',
                          color:
                            domainHealth.dmarc.ok && !domainHealth.dmarc.warn
          ...
          {/* 04 — Email Warm-up */}
          <section
            ref={sectionRefs.warmup}
            style={{ ...s.section, borderBottom: '1px solid var(--border)' }}
          >
            <SectionHeader num="04" title="Email Warm-up" />
            <p style={s.sectionHint}>
              Gradually increase your sending volume to build sender reputation — like Instantly.ai.
              Sonar sends friendly warm-up emails to your partner addresses each day, starting at
              2/day and ramping up to 40/day over 20 days.
            </p>

            {warmupLoading ? (
              <div
                style={{ height: 80, background: 'var(--bg)', borderRadius: 6, marginBottom: 16 }}
              />
            ) : (
              <>
                {warmup?.config && warmup.config.enabled && (
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        Day {warmup.day} / 20 &nbsp;·&nbsp; {warmup.daily_target} emails/day
                      </span>
                    </div>
                    <div
                      style={{
                    ...
                    >
                      <div
                        style={{
                          height: '100%',
                          width: `${Math.min(100, (warmup.day / 20) * 100)}%`,
                          background: '#4a7c59',
                          borderRadius: 2,
                  ...
                  >
                    <span
                      style={{
                        position: 'absolute',
                        top: 3,
                        left: warmupEnabled ? 22 : 3,
                        width: 16,
                        height: 16,
                        borderRadius: '50%',
                        background: warmupEnabled ? '#fff' : '#9CA3AF',
                ...
                <Field
                  ...
                  hint="Comma-separated list of email addresses to send warm-up emails to. Leave blank to automatically use the Sonar warmup network — other platform users who have warm-up enabled."
                >
                  <input
                    value={warmupEmails}
                ...
                {warmupEnabled && !warmupEmails.trim() && (
                  ...
                  >
                    ✓ Auto network active — warm-up emails will be sent to other Sonar users in the
                    pool.
                  </p>
                )}
                ...
                {warmup?.config?.start_date && (
                  ...
                  >
                    ...
                    >
                      Started:{' '}
                      {new Date(warmup.config.start_date).toLocaleDateString(undefined, {
                        weekday: 'short',
                        month: 'short',
                        day: 'numeric',
                        year: 'numeric',
                      ...
                      &nbsp;·&nbsp; Target today:{' '}
                      <strong style={{ color: 'var(--text)' }}>{warmup.daily_target}</strong>{' '}
                      emails/day
                    </p>
                  </div>
                )}
                ...
                <button
                  ...
                  style={{
                    ...
                    fontWeight: 600,
                    padding: '10px 20px',
                    background: warmupSaved ? '#4a7c59' : 'var(--accent)',
                    color: '#fff',
                    border: 'none',
                    borderRadius: 3,
                    cursor: warmupSaving ? 'default' : 'pointer',
                    letterSpacing: '0.04em',
                  }}
                >
                ...
                >
                  ...
                  >
                    Day 1: 2 emails · Day 2: 4 emails · … · Day 20: 40 emails/day.
                    <br />
                    Requires Sequence Automation SMTP configured above. Runs daily at 8 AM UTC.
                    <br />
                    Use your own secondary email accounts as partners to keep warm-up traffic in
                    your control.
                  </p>
                </div>
              </>
          ...
          <section
            ref={sectionRefs.scheduler}
            style={{ ...s.section, borderBottom: '1px solid var(--border)' }}
          >
            <SectionHeader num="04" title="Meeting Scheduler" />
            <p style={s.sectionHint}>
              Paste your Calendly, Cal.com, Google Calendar, or any booking URL. Use{' '}
              <code
                style={{
                  fontFamily: 'var(--font-mono)',
                  fontSize: 11,
              ...
              in any sequence email to insert it automatically.
            </p>
            ...
            <Field
              ...
              hint={`Share as sonarleads.vercel.app/book/${calSlug || 'yourname'}`}
            >
              <div
                style={{
                  display: 'flex',
              ...
              >
                ...
                <input
                  ...
                  value={calSlug}
                  onChange={(e) =>
                    setCalSlug(e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, ''))
                  }
            ...
            <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginTop: 4 }}>
              <button
                ...
                style={{
                  ...
                  fontWeight: 600,
                  padding: '10px 20px',
                  background: calSaved ? '#4a7c59' : 'var(--accent)',
                  color: '#fff',
                  border: 'none',
                  borderRadius: 3,
                  cursor: calSaving ? 'default' : 'pointer',
                  letterSpacing: '0.04em',
                }}
              >
            ...
            {calLink && (
              ...
              >
                ...
                >
                  ...
                  anywhere in your sequence email body. Sonar replaces it with{' '}
                  {calSlug ? (
                    <code style={{ background: 'var(--bg)', padding: '1px 5px', borderRadius: 3 }}>
                      sonarleads.vercel.app/book/{calSlug}
                    </code>
                  ) : (
                    'your booking URL'
                  )}{' '}
          ...
          {/* 06 — Webhooks */}
          <section
            ref={sectionRefs.webhooks}
            style={{ ...s.section, borderBottom: '1px solid var(--border)' }}
          >
            <SectionHeader num="05" title="Webhooks" />
            <p style={s.sectionHint}>
              Send real-time HTTP POST notifications to your endpoints when key events happen in
              Sonar. Compatible with Zapier, Make, n8n, or any custom integration.
            </p>
                      ...
                      >
                        ...
                        <span
                          style={{
                            ...
                            color: wh.active ? '#4a7c59' : 'var(--text-muted)',
                          }}
                        >
            ...
            {whShowForm ? (
              ...
              >
                    ...
                    {ALL_WEBHOOK_EVENTS.map((ev) => (
                      <button
                        ...
                        style={{
                          ...
                          cursor: 'pointer',
                          border: '1px solid',
                          borderColor: whForm.events.includes(ev) ? '#E7000B' : 'var(--border)',
                          background: whForm.events.includes(ev)
                            ? 'rgba(231,0,11,0.1)'
                            : 'var(--surface)',
                          color: whForm.events.includes(ev) ? '#E7000B' : 'var(--text-muted)',
                        }}
                      >
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  <button
                    onClick={addWebhook}
                    disabled={whAdding || !whForm.url || !whForm.events.length}
                    style={{ ...s.saveBtn, opacity: whAdding || !whForm.url ? 0.6 : 1 }}
                  >
                    {whAdding ? 'Adding…' : 'Add webhook'}
                  </button>
                  <button
                    onClick={() => setWhShowForm(false)}
                    style={{ ...s.saveBtn, background: 'var(--surface)', color: 'var(--text)' }}
                  >
                    Cancel
                  </button>
                </div>
            ...
            >
              ...
              >
                1. Create a Zap → Trigger: Webhooks by Zapier → Catch Hook → copy the URL above.
                <br />
                2. Select events you want (e.g.{' '}
                <code style={{ color: 'var(--text)' }}>lead.created</code>).
                <br />
          ...
          {/* 07 — API Keys */}
          <section
            ref={sectionRefs['api-keys']}
            style={{ ...s.section }}
            onMouseEnter={loadApiKeys}
          >
            <SectionHeader num="07" title="API Keys" />
            <p style={s.sectionHint}>
              Create API keys to access the Sonar API from your own applications or integrations.
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
              ...
              <button
                ...
                style={{
                  ...
                  cursor: !newKeyName.trim() || creatingKey ? 'default' : 'pointer',
                  whiteSpace: 'nowrap',
                }}
              >
                  ...
                  >
                    ...
                    <span
                      style={{
                        ...
                        padding: '2px 8px',
                        borderRadius: 4,
                        background: k.active ? '#05966918' : 'rgba(255,255,255,0.04)',
                        color: k.active ? '#059669' : 'var(--text-muted)',
                        border: `1px solid ${k.active ? '#05966940' : 'var(--border)'}`,
                      }}
                    >
          ...
          <section ref={sectionRefs['system-health']} style={{ ...s.section }}>
            ...
            <p style={s.sectionHint}>
              Live status of the background worker and scheduled cron jobs.
            </p>
            ...
            >
              ...
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12 }}>
                ...
                {workerHealth?.status === 'healthy' &&
                  `Worker healthy — last seen ${Math.round(workerHealth.age_seconds)}s ago · watching ${workerHealth.queues_watched ?? '?'} queues`}
                {workerHealth?.status === 'stale' &&
                  `Worker stale — last seen ${Math.round(workerHealth.age_seconds)}s ago`}
                ...
                <tbody>
                  {cronRows.length === 0 && (
                    <tr>
                      ...
                      >
                        No cron data yet — jobs log on first run.
                      </td>
                    </tr>
                  )}
                  {cronRows.map((row) => (
          ...
          <section ref={sectionRefs.privacy} style={{ ...s.section }}>
            ...
            <p style={s.sectionHint}>
              How Sonar collects, uses, and protects your data. Effective June 16, 2026.
            </p>
            ...
            >
              ...
              <p style={s.legalBody}>
                This Privacy Policy describes how Sonar ("Company," "we," "us," or "our") collects,
                uses, and shares information about you when you use our B2B lead intelligence
                platform and Chrome extension (collectively, the "Services").
              </p>

              <p style={s.legalSubhead}>1. Data Collected</p>
              <p style={s.legalBody}>
                We collect your name and email for authentication, B2B company and contact data you
                add or import, anonymised usage analytics via PostHog, and LinkedIn company profile
                data where you authorise API access. We do not collect sensitive personal data such
                as health, financial, or government-issued identification information.
              </p>

              <p style={s.legalSubhead}>2. How We Use It</p>
              <p style={s.legalBody}>
                Your data is used solely to operate the platform: authenticate your account, enrich
                company records via third-party APIs (Hunter, Apollo, Google Maps), sync LinkedIn
                leads into your pipeline, and improve the product via anonymised analytics. We do
                not sell or share your personal data with third parties for advertising purposes.
              </p>

              <p style={s.legalSubhead}>3. Storage & Security</p>
              <p style={s.legalBody}>
                All data is stored in Supabase (PostgreSQL) hosted on AWS with Row Level Security
                enforced — you can only access your own records. All communication is encrypted over
                HTTPS/TLS. Data is retained while your account is active and deleted on request
                within 30 days.
              </p>

              <p style={s.legalSubhead}>4. LinkedIn API Data</p>
              <p style={s.legalBody}>
                LinkedIn data is only accessed with your explicit authorisation via your session
                cookie. Lead data is used solely to populate your pipeline and is never
                redistributed. You can revoke LinkedIn access at any time by removing your cookie
                from Settings.
              </p>

              <p style={s.legalSubhead}>5. Third-Party Services</p>
              <p style={s.legalBody}>
                Supabase (database), Google Maps (location enrichment), PostHog (analytics), Vercel
                (hosting), Hunter.io and Apollo.io (email enrichment). Each service is governed by
              ...
              <p style={s.legalBody}>
                ...
                time. To exercise these rights or for any privacy concerns, contact
                sonarleads@proton.me. We will respond within 30 days.
              </p>

              <p style={s.legalSubhead}>7. Cookies</p>
              <p style={s.legalBody}>
                We use session storage and local storage to maintain your authentication state and
                preferences. No third-party advertising cookies are set.
              </p>
            </div>
            ...
            <ToggleRow
              label={
                <>
                  I have read and agree to Sonar's{' '}
                  <a
                    href="/privacy"
                    target="_blank"
                    rel="noreferrer"
          ...
          <section
            ...
            style={{ ...s.section, borderBottom: 'none', paddingBottom: 0 }}
          >
            <SectionHeader num="07" title="Terms of Service" />
            <p style={s.sectionHint}>
              The rules governing your use of Sonar. Effective June 16, 2026.
            </p>
            ...
            >
              ...
              <p style={s.legalBody}>
                This Terms of Use or Service Agreement ("Agreement") is between Shravan Omanakuttan,
                operating Sonar ("Company," "we," "us," or "our") and the person or entity ("you" or
                "your") that has decided to use our services; any of our websites or apps; or any
                features, products, text, images, data, computer code, and all other forms of data
                and communications (collectively, "Services").
              </p>

              <p style={s.legalBody}>
                YOU MUST CONSENT TO THIS AGREEMENT TO USE OUR SERVICES. If you do not accept and
                agree to be bound by all of the terms of this Agreement, you cannot use Services.
              </p>

              <p style={s.legalBody}>
                If we update this Agreement, we will provide you notice and an opportunity to review
                and decide whether you would like to continue to use the Services.
              </p>

              <p style={s.legalSubhead}>1. Description of the Services</p>
              <p style={s.legalBody}>
                Sonar is a B2B lead intelligence platform that provides tools to discover, enrich,
                and manage company and contact records for sales and marketing purposes.
              </p>

              <p style={s.legalSubhead}>2. Acceptable Use</p>
              <p style={s.legalBody}>
                Use the platform only for lawful B2B sales and marketing purposes. You must not
                scrape third-party platforms in violation of their terms, send unsolicited bulk
                communications, resell platform access, or use the platform to store sensitive
                personal data such as health or financial information.
              </p>

              <p style={s.legalSubhead}>3. Your Data</p>
              <p style={s.legalBody}>
                You own all company, contact, and lead data you create in the platform. You grant us
                a limited licence to store and process it solely to provide the Service. You are
                responsible for ensuring you have the right to upload any data you add.
              </p>

              <p style={s.legalSubhead}>4. LinkedIn API</p>
              <p style={s.legalBody}>
                By connecting LinkedIn, you authorise Sonar to access LinkedIn data on your behalf
                within the approved scope. LinkedIn data may only be used for your own sales
                activities and is subject to LinkedIn's own terms in addition to ours.
              </p>

              <p style={s.legalSubhead}>5. Disclaimers</p>
              <p style={s.legalBody}>
                The platform is provided "as is" without warranties of any kind. We do not guarantee
                that enrichment data from third-party APIs will be accurate, complete, or current.
                The platform may be unavailable during maintenance windows.
              </p>

              <p style={s.legalSubhead}>6. Limitation of Liability</p>
              <p style={s.legalBody}>
                Our total liability for any claims is limited to the amount you paid us in the prior
                12 months or ₹5,000, whichever is greater. We are not liable for indirect,
                incidental, or consequential damages including loss of business or data.
              </p>
            </div>
          ...
          <div style={{ paddingTop: '32px', borderTop: '1px solid var(--border)' }}>
            <button
              ...
              style={{
                ...
                width: '100%',
                padding: '14px 20px',
                background: saved ? '#4a7c59' : 'var(--accent)',
                border: 'none',
                borderRadius: 0,
                fontFamily: 'var(--font-mono)',
                fontSize: '12px',
  ...
  return (
    <a
      ...
      style={{
        ...
        textDecorationColor: 'rgba(168,100,72,0.35)',
      }}
    >
      {children}
    </a>
...
const badge = {
  ...
  blue: {
    ...
    padding: '2px 7px',
    borderRadius: 0,
    border: '1px solid rgba(0,130,243,0.3)',
    letterSpacing: '0.06em',
    whiteSpace: 'nowrap',
  },
  amber: {
