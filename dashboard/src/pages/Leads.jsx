...
const DECISION_MAKER_KEYWORDS =
  /\b(ceo|cto|ciso|coo|cfo|founder|co-founder|cofounder|president|vice president|vp|director|head of|chief|managing director|md|partner|principal|owner|general manager|gm|svp|evp|senior vice|senior director)\b/i
...
function BulkActionBar({
  ...
  const btn = (extra = {}) => ({
    ...bulk.actionBtn,
    ...(loading ? { opacity: 0.45, pointerEvents: 'none' } : {}),
    ...extra,
  })
  ...
  return (
    <div style={bulk.bar}>
      <div style={bulk.left}>
        ...
        <button style={bulk.clearBtn} onClick={onClearSelection}>
          Clear
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div style={bulk.actions}>
          ...
          <button style={btn()} onClick={onScoreICP}>
            Score ICP
          </button>
          <button style={btn()} onClick={onFindEmails}>
            Find Emails
          </button>
          <button
            style={btn({ color: '#10b981', borderColor: 'rgba(16,185,129,0.3)' })}
          ...
          <div style={{ position: 'relative' }}>
            <button style={btn()} onClick={openSequences}>
              Sequence ↓
            </button>
            {showSequences && (
              <div
              ...
              >
                {sequences.length === 0 ? (
                  <div
                    style={{ ...bulk.dropdownItem, color: 'var(--text-muted)', cursor: 'default' }}
                  >
                    No sequences yet
                  </div>
                ) : (
                  sequences.map((seq) => (
                    <button
                      key={seq.id}
                      style={bulk.dropdownItem}
                      onClick={() => {
                        onEnrollSequence(seq.id)
                        closeAll()
                      }}
                    >
                      {seq.name}
          ...
          <button
            style={btn({ color: '#ef4444', borderColor: 'rgba(239,68,68,0.3)' })}
            onClick={onDelete}
          >
            Delete
          </button>
  ...
  return React.createElement(
    'button',
    {
      ...
      style: {
        ...
        cursor: 'pointer',
        fontSize: '14px',
        color: starred ? 'var(--accent)' : 'var(--border-strong)',
        padding: '2px 4px',
...
function IcpScoreBadge({ score, reason }) {
  ...
  const bg =
    score >= 70
      ? 'rgba(74,124,89,0.10)'
      : score >= 40
        ? 'rgba(168,100,72,0.10)'
        : 'rgba(161,161,161,0.08)'
  const border =
    score >= 70
      ? 'rgba(74,124,89,0.3)'
      : score >= 40
...
function EmailVerificationBadge({ status, score, reason }) {
  ...
  return (
    <span
      title={
        reason
          ? `${reason}${score != null ? ` · ${score}% deliverability` : ''}`
          : score != null
    ...
    React.createElement(
      'div',
      {
        style: {
          ...
          maxWidth: '600px',
          padding: '28px',
          boxShadow: '0 20px 60px rgba(29,27,27,0.2)',
          maxHeight: '90vh',
          overflowY: 'auto',
        },
        onClick: (e) => e.stopPropagation(),
      ...
      React.createElement(
        'button',
        {
          ...
          style: {
            width: '100%',
            padding: '12px',
            background: copied ? '#4a7c59' : 'var(--text)',
            color: '#FFFFFF',
            border: 'none',
            borderRadius: '7px',
            fontFamily: 'var(--font-mono)',
...
function LeadRow({
  ...
  return (
    <div
      style={{
        ...s.trow,
        background: isSelected ? 'rgba(168,100,72,0.07)' : 'transparent',
        borderLeft: isSelected
          ? '2px solid var(--accent)'
          : lead.starred
            ? '2px solid var(--accent)'
            : '2px solid transparent',
        outline: isSelected ? '1px solid rgba(168,100,72,0.18)' : 'none',
        outlineOffset: -1,
      }}
    >
      ...
      {columns.map((col) => {
        const isEditing =
          editingCell && editingCell.leadId === lead.id && editingCell.field === col.key
        return (
          <div
            key={col.key}
            style={{
              ...s.td,
              flex: col.flex,
              cursor: col.key === 'name' && onRowClick ? 'pointer' : 'text',
            }}
            onClick={() =>
              col.key === 'name' && onRowClick
                ? onRowClick(lead)
                : onStartEdit(lead.id, col.key, lead[col.key])
            }
          >
            ...
            {!isEditing && col.key === 'email' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 3, minWidth: 0 }}>
                  ...
                  {lead.email && lead.email_provider && (
                    <span
                      style={{
                        ...
                        flexShrink: 0,
                        background:
                          lead.email_provider === 'web'
                            ? 'rgba(139,92,246,0.12)'
                            : lead.email_provider === 'smtp'
                              ? 'rgba(16,185,129,0.12)'
                              : lead.email_provider === 'site'
                ...
                {lead.email && (
                  <EmailVerificationBadge status={lead.email_status} score={lead.email_score} />
                )}
              </div>
            )}
            {!isEditing && col.key !== 'status' && col.key !== 'name' && col.key !== 'email' && (
      ...
      >
        ...
        {!lead.email && (
          <button
            style={{
              ...
              fontSize: '10px',
              letterSpacing: '0.04em',
              color: isEnriching ? 'var(--text-muted)' : 'var(--accent)',
              background: 'transparent',
              border: '1px solid var(--border)',
              borderRadius: '4px',
              cursor: 'pointer',
        ...
        <button
          title={
            lead.icp_score != null
              ? `ICP score: ${lead.icp_score} — click to refresh`
              : 'Score against ICP'
          }
          style={{
            fontFamily: 'var(--font-mono)',
            fontSize: '10px',
            letterSpacing: '0.04em',
            color: isScoring ? 'var(--text-muted)' : '#5b8db8',
            background: 'transparent',
            border: '1px solid rgba(91,141,184,0.3)',
            borderRadius: '4px',
            cursor: 'pointer',
        ...
        <button
          ...
          style={{
            ...
            fontSize: '10px',
            letterSpacing: '0.04em',
            color: isDrafting ? 'var(--text-muted)' : '#4a7c59',
            background: 'transparent',
...
export default function Leads() {
  ...
  async function fetchLeads() {
    try {
      ...
      getUnverifiedCount()
        .then((r) => setUnverifiedCount(r.data?.count || 0))
        .catch(() => {})
    } catch (e) {
      console.error(e)
      setLeads([])
  ...
  async function saveEdit(leadId) {
    ...
    try {
      ...
      setLeads((prev) =>
        prev.map((l) => (l.id === leadId ? { ...l, [editingCell.field]: editValue } : l))
      )
    } catch (e) {
      console.error(e)
    } finally {
  ...
  async function handleBulkDelete() {
    if (
      !window.confirm(
        `Delete ${selected.length} lead${selected.length !== 1 ? 's' : ''}? This cannot be undone.`
      )
    )
      return
    const ids = [...selected]
  ...
  async function handleBulkStatus(status) {
    setBulkLoading(true)
    setBulkLabel(`Setting status to ${status}…`)
    try {
  ...
  async function handleBulkConnectionStatus(connection_status) {
    setBulkLoading(true)
    setBulkLabel(`Updating connection status…`)
    try {
      await Promise.all(selected.map((id) => updateConnectionStatus(id, connection_status)))
      setLeads((prev) =>
        prev.map((l) => (selected.includes(l.id) ? { ...l, connection_status } : l))
      )
      setSelected([])
    } catch (e) {
      console.error(e)
  ...
  async function handleBulkEnrollSequence(seqId) {
    setBulkLoading(true)
    setBulkLabel(`Enrolling ${selected.length} leads…`)
    try {
      await enrollInSequence(seqId, selected)
      setImportMsg(
        `Enrolled ${selected.length} lead${selected.length !== 1 ? 's' : ''} in sequence.`
      )
      setSelected([])
  ...
  async function handleBulkScoreICP() {
    setBulkLoading(true)
    setBulkLabel(`Scoring ${selected.length} leads against ICP…`)
    try {
  ...
  async function handleSegEnrollSequence(seqId, seqName) {
    ...
    try {
      ...
      setImportMsg(
        `Enrolled ${filtered.length} lead${filtered.length !== 1 ? 's' : ''} in ${seqName}.`
      )
      setTimeout(() => setImportMsg(''), 3000)
    } catch (e) {
      console.error(e)
      setImportMsg('Enrollment failed.')
    } finally {
      setSegEnrolling(false)
    }
  }
  ...
  async function handleBulkFindEmails() {
    ...
    if (!needEmail.length) {
      setImportMsg('All selected leads already have emails.')
      setTimeout(() => setImportMsg(''), 3000)
      return
    }
    setBulkLoading(true)
    setBulkLabel(`Finding emails for ${needEmail.length} leads…`)
    try {
      ...
      setImportMsg(
        `Found ${enriched} email${enriched !== 1 ? 's' : ''} out of ${needEmail.length} attempted.`
      )
      setSelected([])
      setTimeout(() => setImportMsg(''), 5000)
    } catch (e) {
      console.error(e)
      setImportMsg('Email enrichment failed.')
    } finally {
      setBulkLoading(false)
      setBulkLabel('')
    }
  ...
  async function handleBulkVerifyEmails(ids) {
    ...
    if (!targets.length) {
      setImportMsg('No emails to verify in selection.')
      setTimeout(() => setImportMsg(''), 3000)
      return
    }
    setBulkLoading(true)
    setBulkLabel(`Verifying ${targets.length} emails…`)
    try {
      ...
      ;(res.data.results || []).forEach((r) => {
        if (r.status !== 'skipped' && r.status !== 'error') {
          setLeads((prev) =>
            prev.map((l) =>
              l.id === r.lead_id ? { ...l, email_status: r.status, email_score: r.score } : l
            )
          )
        }
      })
      setImportMsg(
        `Verified ${summary.verified}: ${summary.valid} valid · ${summary.risky} risky · ${summary.invalid} invalid · ${summary.role} role`
      )
      ...
      console.error(e)
      setImportMsg('Verification failed.')
    } finally {
      setBulkLoading(false)
      setBulkLabel('')
    }
  ...
  async function handleImportCSV(e) {
    ...
    try {
      ...
      setImportMsg(
        `Imported ${created} lead${created !== 1 ? 's' : ''}${skipped ? `, ${skipped} skipped (duplicates)` : ''}.`
      )
      fetchLeads()
    } catch (e) {
  ...
  async function handleConnectionStatus(leadId, connection_status) {
    ...
    try {
      ...
      console.error(e)
      setLeads((prev) =>
        prev.map((l) => (l.id === leadId ? { ...l, connection_status: prev_status } : l))
      )
    }
  }
  ...
  async function handleScoreICP(leadId) {
    ...
    try {
      ...
      setLeads((prev) =>
        prev.map((l) =>
          l.id === leadId ? { ...l, icp_score: score, icp_score_reason: reason } : l
        )
      )
    } catch (e) {
      console.error(e)
  ...
  async function handleDraftEmail(leadId) {
    ...
    try {
      ...
      setLeads((prev) =>
        prev.map((l) =>
          l.id === leadId
            ? { ...l, email_draft: res.data.body, email_draft_subject: res.data.subject }
            : l
        )
      )
    } catch (e) {
  ...
  const filtered = leads.filter((l) => {
    ...
    const msen =
      !filterSeniority ||
      (SENIORITY_MAP[filterSeniority] && SENIORITY_MAP[filterSeniority].test(l.title || ''))
    ...
    const memail =
      !filterEmailStatus ||
      (filterEmailStatus === 'unverified'
        ? !l.email_status && l.email
        : l.email_status === filterEmailStatus)
  ...
  function handleExport(leadsToExport) {
    const toExport =
      leadsToExport ||
      (selected.length ? filtered.filter((l) => selected.includes(l.id)) : filtered)
  ...
  const decisionMakerCount = leads.filter((l) =>
    DECISION_MAKER_KEYWORDS.test((l.title || '').toLowerCase())
  ).length
  const securityCount = leads.filter((l) =>
    SECURITY_KEYWORDS.test((l.title || '').toLowerCase())
  ).length
  ...
  return (
    <div style={s.page}>
      ...
      >
        ...
        >
          {(enrichMsg || scoreMsg) && (
            <span style={{ fontSize: '12px', color: '#4a7c59', fontWeight: '500' }}>
              {scoreMsg || enrichMsg}
            </span>
          )}
          <button
            style={{
              ...s.starFilterBtn,
              background: starredOnly ? 'var(--accent)' : 'transparent',
              color: starredOnly ? 'var(--bg)' : 'var(--accent)',
              borderColor: 'rgba(168,100,72,0.4)',
            }}
          ...
          <button
            ...
            style={{
              padding: '7px 14px',
              background: scoringAll ? 'var(--surface)' : '#1d1b1b',
              border: '1px solid var(--border)',
              borderRadius: 7,
              fontFamily: 'var(--font-mono)',
              fontSize: 11,
              fontWeight: 600,
              color: '#fff',
              cursor: scoringAll ? 'default' : 'pointer',
              opacity: scoringAll ? 0.6 : 1,
            }}
          >
          ...
          <button style={s.spreadsheetBtn} onClick={() => setShowSpreadsheet(true)}>
            Spreadsheet
          </button>
          <input
            ref={csvInputRef}
            ...
            >
              ...
              >
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button
                    onClick={() => setExportCols(ALL_EXPORT_COLS.map((c) => c.key))}
                    style={{
                      fontFamily: 'var(--font-mono)',
                      fontSize: 10,
                      color: 'var(--text-muted)',
                ...
                {ALL_EXPORT_COLS.map((col) => (
                  ...
                  >
                    <input
                      ...
                      style={{ cursor: 'pointer', accentColor: '#E7000B' }}
                    />
                    <span
              ...
              >
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 7,
                    color: '#fff',
                    cursor: exportCols.length ? 'pointer' : 'default',
                    opacity: exportCols.length ? 1 : 0.5,
                  }}
                >
                  Export{' '}
                  {selected.length ? `${selected.length} selected` : `${filtered.length} leads`}
                </button>
              </div>
            </div>
          </>
        ...
        {importMsg && (
          <div
            style={{
              padding: '8px 24px',
              background: importMsg.includes('failed')
                ? 'rgba(231,0,11,0.08)'
                : 'rgba(74,124,89,0.1)',
              borderBottom: '1px solid var(--border)',
            }}
          >
            <span
              style={{
                fontFamily: 'var(--font-mono)',
                fontSize: 11,
                color: importMsg.includes('failed') ? '#E7000B' : '#4a7c59',
              }}
            >
              {importMsg}
            </span>
      ...
      <div style={s.container}>
        ...
        {unverifiedCount > 0 && !verifyBannerDismissed && (
          ...
          >
            ...
            >
              <strong>{unverifiedCount}</strong> lead{unverifiedCount !== 1 ? 's have' : ' has'} an
              email with no verification status
            </span>
            <button
              onClick={() => {
              ...
              >
                ...
                <span
                  style={{
                    ...
                    fontSize: 11,
                    fontWeight: 700,
                    color: score >= 70 ? '#4a7c59' : score >= 40 ? '#b07d2e' : '#e07070',
                    flexShrink: 0,
                  }}
                >
                  {score}%
          ...
          <div style={{ display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            {[
              ...
              <button
                ...
                style={{
                  ...s.viewTab,
                  ...(viewFilter === tab.key && !activeSegmentId ? s.viewTabActive : {}),
                }}
              >
                {tab.label}
                <span
                  style={{
                    ...s.viewTabCount,
                    background:
                      viewFilter === tab.key && !activeSegmentId
                        ? 'rgba(29,27,27,0.10)'
                        : 'var(--surface-raised)',
                    color:
                      viewFilter === tab.key && !activeSegmentId
            ...
            {segments.map((seg) => (
              <button
                ...
                style={{
                  ...s.viewTab,
                  ...(activeSegmentId === seg.id ? s.viewTabActive : {}),
                  display: 'flex',
                  alignItems: 'center',
                  gap: 5,
                }}
              ...
              <div style={{ position: 'relative', marginLeft: 6 }}>
                <button
                  ...
                  style={{
                    padding: '5px 12px',
                    background: 'transparent',
                    border: '1px solid rgba(168,100,72,0.4)',
                    borderRadius: 6,
                    fontFamily: 'var(--font-mono)',
                    fontSize: 10,
                    fontWeight: 600,
                    color: segEnrolling ? 'var(--text-muted)' : 'var(--accent)',
                    cursor: segEnrolling || filtered.length === 0 ? 'default' : 'pointer',
                    whiteSpace: 'nowrap',
                    letterSpacing: '0.04em',
                    opacity: segEnrolling ? 0.6 : 1,
                  }}
                >
            ...
            {showSegmentInput ? (
              <>
                ...
                <button
                  ...
                  disabled={savingSegment || !segmentNameInput.trim()}
                  style={{
                    padding: '5px 12px',
                    ...
                    color: '#fff',
                    cursor: 'pointer',
                    opacity: savingSegment || !segmentNameInput.trim() ? 0.5 : 1,
                  }}
                >
                  {savingSegment ? '…' : 'Save'}
                </button>
              ...
              hasActiveFilters && (
                <button
                  onClick={() => setShowSegmentInput(true)}
                  style={{
                    padding: '5px 12px',
                    background: 'transparent',
                    border: '1px solid var(--border)',
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <button
              ...
              style={{
                padding: '5px 12px',
                borderRadius: 6,
                border: `1px solid ${showAdvanced || filterSeniority || filterConnection || filterMinScore ? 'var(--accent)' : 'var(--border)'}`,
                background: 'transparent',
                fontFamily: 'var(--font-mono)',
                fontSize: 11,
            ...
            <button
              ...
              style={{
                padding: '5px 12px',
                borderRadius: 6,
                border: `1px solid ${sortBy === 'hottest' ? 'rgba(202,138,4,0.5)' : 'var(--border)'}`,
                background: sortBy === 'hottest' ? 'rgba(234,179,8,0.08)' : 'transparent',
                fontFamily: 'var(--font-mono)',
                fontSize: 11,
                color: sortBy === 'hottest' ? '#ca8a04' : 'var(--text-muted)',
                cursor: 'pointer',
                fontWeight: sortBy === 'hottest' ? 700 : 400,
              }}
            >
        ...
        {viewMode === 'pipeline' && (
          <PipelineView leads={filtered} onLeadClick={setSelectedLead} onLeadsUpdate={setLeads} />
        )}
        ...
        {viewMode === 'table' && (
          <div style={s.table}>
            <div style={s.thead}>
              ...
              {columns.map((col) => (
                <span key={col.key} style={{ ...s.th, flex: col.flex }}>
                  {col.label}
                </span>
              ))}
            ...
            {loading &&
              [...Array(8)].map((_, i) => (
                <SkeletonRow key={i} cols={[0.3, 0.5, 2, 2, 2, 1.5, 1.5, 1]} />
              ))}
            {!loading && filtered.length === 0 && (
              <div style={{ padding: '72px 20px', textAlign: 'center' }}>
                ...
                >
                  {viewFilter !== 'all'
                    ? 'Try switching to All leads or adjusting filters.'
                    : leads.length === 0
...
const s = {
  ...
  trow: {
    display: 'flex',
    padding: '0 20px',
    borderTop: '1px solid rgba(196,193,189,0.35)',
    alignItems: 'center',
    minHeight: '50px',
...
const bulk = {
  ...
  selectAllBtn: {
    background: 'none',
    border: '1px solid rgba(168,100,72,0.3)',
    borderRadius: 5,
    fontFamily: 'var(--font-mono)',
    color: 'var(--accent)',
    fontSize: '10px',
