          ...
          <div key={col.key} style={{ ...s.td, flex: col.flex, cursor: col.key === 'name' && onRowClick ? 'pointer' : 'text' }} onClick={() => col.key === 'name' && onRowClick ? onRowClick(lead) : onStartEdit(lead.id, col.key, lead[col.key])}>
            ...
            {!isEditing && col.key === 'name' && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <span style={{ fontSize: '13px', color: 'var(--text)', fontWeight: '500', textDecoration: onRowClick ? 'underline' : 'none', textDecorationColor: 'var(--border-strong)', textUnderlineOffset: 3 }}>{lead.name || '—'}</span>
                {lead.source === 'linkedin_lead_gen' && (
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="#0077B5" title="LinkedIn Lead Gen">
