...
export default function ICP() {
  ...
  const [savedICPs, setSavedICPs] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem(icpKey) || localStorage.getItem('savedICPs') || '[]')
    } catch {
      return []
    }
  })
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <div style={{ padding: '40px 32px 24px', borderBottom: '1px solid var(--border)' }}>
        ...
        <p style={{ fontSize: '13px', color: 'var(--text-muted)', marginTop: '8px' }}>
          Filter your leads by ideal customer criteria and find the best matches.
        </p>
      </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginTop: '8px' }}>
              {DM_TITLES.map((t) => (
                <button
                  key={t}
                  onClick={() => toggle(selectedTitles, setSelectedTitles, t)}
                  style={{
                    padding: '5px 10px',
                    borderRadius: '6px',
                    fontSize: '11px',
          ...
          <div>
            ...
            >
              <input
                ...
                onChange={(e) => setRequireProduct(e.target.checked)}
                style={{ accentColor: 'var(--accent)', cursor: 'pointer' }}
              />
              <span style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>
            ...
            {showSaveForm ? (
              ...
              <button
                ...
                style={{
                  width: '100%',
                  padding: '9px',
                  background: hasFilters ? 'var(--surface)' : 'transparent',
                  border: '1px solid var(--border)',
                  borderRadius: '8px',
                  fontSize: '12px',
                  color: hasFilters ? 'var(--text-secondary)' : 'var(--text-muted)',
        ...
        <div style={{ padding: '24px', overflowY: 'auto' }}>
          ...
          {hasFilters && leads.length > 0 && (
            <div style={{ marginBottom: '20px' }}>
              ...
              >
                ...
                <span style={{ fontSize: '11px', color: '#4a7c59', fontWeight: '600' }}>
                  {Math.round((matched.length / leads.length) * 100)}%
                </span>
              </div>
              <div
                style={{
              ...
              >
                <div
                  style={{
                    height: '100%',
                    width: `${Math.round((matched.length / leads.length) * 100)}%`,
                    background: '#4a7c59',
                    borderRadius: '3px',
          ...
          {!hasFilters ? (
            <div style={{ padding: '80px 0', textAlign: 'center' }}>
              ...
              >
                Select decision maker titles, org sizes, and requirements on the left to see which
            ...
            <div style={{ padding: '60px 0', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: '13px', color: 'var(--text-muted)' }}>
                Try broadening your ICP criteria.
              </p>
            </div>
          ) : (
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      padding: '14px 16px',
                      background: 'var(--surface)',
                      border: `1px solid ${isDM ? 'rgba(168,100,72,0.25)' : 'var(--border)'}`,
                      borderRadius: '10px',
                      borderLeft: `3px solid ${isDM ? 'var(--accent)' : 'transparent'}`,
                    }}
                  >
                    <div>
                      <p
