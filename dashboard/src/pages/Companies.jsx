...
import { getCompanies, updateCompany, deleteCompany, getCompanyLeads } from '../services/api'
...
const prospectColors = {
  'To Review': 'var(--gray-4)',
  'Prospect': 'var(--green)',
  'Not a Fit': '#ff2d2d',
  'Contacted': 'var(--amber)',
  'In Progress': '#00d4ff',
  'Closed Won': 'var(--green)',
  'Closed Lost': 'var(--gray-4)',
}
      ...
      <div style={card.footer}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          {company.linkedin_url && (
            <a href={company.linkedin_url} style={card.linkedinBtn}>
              LinkedIn →
            </a>
          )}
          <button
            style={card.findBtn}
            onClick={() => onViewLeads(company)}
          >
            View / Find Leads →
          </button>
        </div>
...
function EmptyLeadsLink({ company }) {
  ...
  return (
    <a href={peopleUrl} style={{ color: 'var(--green)', fontSize: '13px', fontWeight: '600' }}>
        ...
        {loading ? (
         ...
         <div style={modal.emptyState}>
            ...
            <p style={modal.emptyText}>
              Go to the company LinkedIn people page and use the extension to extract leads.
            </p>
          ...
          <div style={modal.leadsGrid}>
            {leads.map(lead => (
              <div key={lead.id} style={modal.leadCard}>
                ...
                {lead.profile_url && (
                  <a href={lead.profile_url} style={modal.leadLink}>View Profile →</a>
                )}
              </div>
            ))}
          </div>
      ...
      <div style={s.hero}>
        <div>
          ...
          <div style={s.heroStats}>
            ...
            <span style={{ color: '#ff2d2d', fontSize: '12px', fontWeight: '600' }}>
      ...
      <div style={s.container}>
        ...
        {loading ? (
          ...
          <div style={s.emptyState}>
            ...
            <p style={s.emptyText}>
              Use the extension on LinkedIn company search or Sales Navigator to extract companies.
            </p>
          </div>
        ) : (
          <div style={s.grid}>
...
const card = {
  wrapper: { background: 'var(--gray-1)', border: '1px solid var(--gray-2)', borderRadius: '4px', overflow: 'hidden', display: 'flex', flexDirection: 'column', gap: '0' },
  ...
  footer: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 20px' },
