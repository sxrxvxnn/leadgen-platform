...
export default function Unsubscribes() {
  ...
  async function handleAdd() {
    ...
    if (items.some((i) => i.email === email)) {
      setAddError('Already suppressed.')
      return
    }
    setAdding(true)
    setAddError('')
  ...
  function handleExport() {
    const csv = [
      ...
      ...filtered.map((i) => `"${i.email}","${i.created_at}"`),
    ].join('\n')
    const a = document.createElement('a')
    a.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv' }))
    a.download = 'sonar-unsubscribes.csv'
    a.click()
  }
    ...
    <div style={s.page}>
      ...
      >
        <div>
          ...
          <p style={s.heroSub}>
            People who have opted out of your Sonar sequences. They will never be emailed again by
            any sequence.
          </p>
        </div>
      ...
      {msg && (
        <div
          style={{
            padding: '10px 48px',
            background: msg.startsWith('✓') ? 'rgba(74,124,89,0.08)' : 'rgba(231,0,11,0.06)',
        ...
        >
          <span
            style={{
              ...
              fontSize: 11,
              color: msg.startsWith('✓') ? '#4a7c59' : '#E7000B',
            }}
          >
            {msg}
          </span>
      ...
      <div style={s.container}>
        ...
        <div style={s.addCard}>
          ...
          <p style={s.addHint}>
            Useful for adding emails from opt-out requests received outside Sonar (e.g. direct
            replies).
          </p>
        </div>
            ...
            <div style={{ padding: '72px 24px', textAlign: 'center' }}>
              ...
              >
                {search
                  ...
                  : 'When someone clicks unsubscribe in a sequence email, they appear here.'}
              </p>
            </div>
          )}
          ...
          {!loading &&
            filtered.map((item, i) => (
              ...
              >
                ...
                >
                  {item.unsubscribed_at || item.created_at
                    ? new Date(item.unsubscribed_at || item.created_at).toLocaleDateString(
                        'en-US',
                ...
                <span style={{ ...s.td, flex: 2 }}>
                  <span
                    style={{
                      ...
                      color: item.reason === 'hard_bounce' ? '#e07070' : 'var(--text-muted)',
                      border:
                        item.reason === 'hard_bounce'
                          ? '1px solid rgba(224,112,112,0.3)'
        ...
        <div style={s.infoBox}>
          ...
          <ul style={s.infoList}>
            <li>
              When a lead clicks "Unsubscribe" in any sequence email, they are added here
              automatically.
            </li>
            <li>
              Sonar checks this list before sending every sequence step — suppressed emails are
              silently skipped.
            </li>
            <li>Active enrollments for suppressed emails are paused immediately.</li>
            <li>
              Removing a suppression re-enables them for future sequences, but does not re-enroll
              them in any existing sequence.
            </li>
          </ul>
        </div>
      </div>
...
const s = {
  ...
  row: {
    display: 'flex',
    padding: '12px 20px',
    borderTop: '1px solid rgba(196,193,189,0.25)',
    alignItems: 'center',
