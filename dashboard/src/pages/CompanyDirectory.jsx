  ...
  useEffect(() => {
    ...
    if (bounds.length && mapRef.current) {
      ...
      if (allBounds.length > 1)
        mapRef.current.fitBounds(allBounds, { padding: [40, 40], maxZoom: 14 })
  ...
  return (
    <div
      style={{
        ...
        boxShadow: '0 2px 12px rgba(29,27,27,0.06)',
      }}
    >
        ...
        >
          <div
            style={{
              ...
              border: '1px solid var(--border)',
              borderRadius: '8px',
              boxShadow: '0 4px 16px rgba(29,27,27,0.10)',
            }}
          >
            <span
              style={{
...
export default function CompanyDiscovery() {
  ...
  async function pickSuggestion(s) {
    ...
    try {
      ...
      setEnriched({
        name: s.name,
        website_url: s.domain ? `https://${s.domain}` : '',
        linkedin_data: {},
      })
    } finally {
      setEnriching(false)
  ...
  async function addToPipeline() {
    ...
    try {
      ...
      alert(
        msg.includes('duplicate') || msg.includes('already') ? 'Already in your pipeline.' : msg
      )
    }
  }
  ...
  async function handleDiscover() {
    if (!discQuery.trim()) {
      setDiscError('Enter a search query.')
      return
    }
    setDiscLoading(true)
    setDiscError('')
    ...
    try {
      ...
      if (!res.data.results?.length)
        setDiscError('No results — try a broader query or different location.')
    } catch (e) {
  ...
  async function discAddSelected() {
    ...
    try {
      const payload = toAdd.map((r) => ({
        ...
        headquarters: r.address || null,
      }))
      const res = await bulkCreateCompanies(payload)
      const { inserted, updated } = res.data
      setDiscMsg(
        `✓ ${inserted} added${updated ? ` · ${updated} updated` : ''} — use Fill All to enrich.`
      )
      setDiscSelected(new Set())
    } catch (e) {
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', position: 'relative' }}>
      <div
        style={{
          position: 'relative',
          zIndex: 1,
          maxWidth: mode === 'discover' ? '860px' : '720px',
          margin: '0 auto',
      ...
      >
        ...
        <p style={s.heroSub}>
          Search millions of companies by name, or discover businesses on a live map using Google
          Maps.
        </p>
        ...
        >
          <button
            ...
            style={{
              ...s.modeBtn,
              background: mode === 'search' ? '#1d1b1b' : 'transparent',
              color: mode === 'search' ? '#fdfdfd' : 'var(--text-muted)',
            }}
          >
            Search by name
          </button>
          <button
            ...
            style={{
              ...s.modeBtn,
              background: mode === 'discover' ? '#2a4a35' : 'transparent',
              color: mode === 'discover' ? '#fdfdfd' : '#4a7c59',
            }}
          >
            ⊕ Discover nearby
          </button>
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                        {selected?.logo ? (
                          ...
                          <div style={s.cardLogoPlaceholder}>
                            {(enriched.name || selected?.name || '?')[0]}
                          </div>
                        )}
                        <div>
                          <p style={s.cardName}>{enriched.name || selected?.name}</p>
                          <p style={s.cardDomain}>
                            {selected?.domain ||
                      ...
                      <div style={s.statsGrid}>
                        {li.followers && (
                          <div style={s.statCell}>
                            <p style={s.statValue}>
                              {fmtNumber(String(li.followers).replace(/[^0-9]/g, ''))}
                            </p>
                    ...
                    {!li.followers &&
                      ...
                      !li.description && (
                        <div style={{ padding: '16px 0', borderTop: '1px solid var(--border)' }}>
                          ...
                          >
                            No LinkedIn data found — you can still add this company and fill details
                            manually.
                          </p>
                        </div>
                      )}
            ...
            {!selected && (
              ...
              >
                {[
                  ...
                  <div
                    key={label}
                    style={{ background: 'var(--bg)', padding: '24px', textAlign: 'center' }}
                  >
                    <p
                      style={{
          ...
          <>
            ...
            >
              ...
              <input
                ...
                step="5"
                value={discRadius}
                onChange={(e) => setDiscRadius(Number(e.target.value))}
                style={{ flex: 1, accentColor: '#4a7c59', cursor: 'pointer' }}
              />
              <button
                onClick={handleDiscover}
                disabled={discLoading || !discQuery.trim()}
                style={{
                  ...s.addBtn,
                  background: '#2a4a35',
                  opacity: discLoading || !discQuery.trim() ? 0.5 : 1,
                  whiteSpace: 'nowrap',
                  flexShrink: 0,
                }}
              >
            ...
            {discResults.length > 0 && (
              <div style={{ marginTop: '16px' }}>
                ...
                >
                  ...
                  <button
                    onClick={() =>
                      setDiscSelected(new Set(discResults.map((r) => r.domain || r.name)))
                    }
                    style={s.linkBtn}
                  >
                    Select all
                  ...
                  {discSelected.size > 0 && (
                    <button onClick={() => setDiscSelected(new Set())} style={s.linkBtn}>
                      Clear
                    </button>
                  )}
                </div>
                          ...
                          >
                            <div
                              style={{
                                ...
                                height: '8px',
                                borderRadius: '50%',
                                background: checked ? '#4a7c59' : 'rgba(168,100,72,0.5)',
                                flexShrink: 0,
                              }}
                            />
                          </div>
                ...
                {discMsg && (
                  <div
                    style={{
                      fontFamily: 'var(--font-mono)',
                      fontSize: '11px',
                      color: discMsg.startsWith('✓') ? '#4a7c59' : 'var(--red)',
                      padding: '8px 12px',
                      background: discMsg.startsWith('✓')
                        ? 'rgba(74,124,89,0.06)'
                        : 'rgba(184,50,50,0.06)',
                      borderRadius: '7px',
                      border: `1px solid ${discMsg.startsWith('✓') ? 'rgba(74,124,89,0.2)' : 'rgba(184,50,50,0.15)'}`,
                      marginBottom: '10px',
                    }}
                  >
                    {discMsg}
                ...
                <button
                  ...
                  disabled={discSelected.size === 0 || discAdding}
                  style={{
                    ...s.addBtn,
                    background: '#2a4a35',
                    width: '100%',
                    textAlign: 'center',
                    opacity: discSelected.size === 0 || discAdding ? 0.45 : 1,
                  }}
                >
            ...
            {!discLoading && discResults.length === 0 && !discError && (
              ...
              >
                {[
                  { value: 'Maps', label: 'Powered by Google' },
                  { value: '20', label: 'Results per search' },
                  { value: 'Live', label: 'Real map with pins' },
                ].map(({ value, label }) => (
                  <div
                    key={label}
                    style={{ background: 'var(--bg)', padding: '24px', textAlign: 'center' }}
...
const s = {
  ...
  dropdown: {
    ...
    border: '1px solid var(--border)',
    borderRadius: '10px',
    boxShadow: '0 8px 32px rgba(29,27,27,0.10)',
    zIndex: 50,
    overflow: 'hidden',
  },
  suggItem: {
  ...
  card: {
    ...
    borderRadius: '10px',
    padding: '24px',
    boxShadow: '0 2px 16px rgba(29,27,27,0.06)',
  },
  cardLogo: {
    width: '44px',
    height: '44px',
