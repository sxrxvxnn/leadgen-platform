...
const roleC = (r) =>
  r === 'owner'
    ? { bg: 'rgba(231,0,11,0.12)', c: '#E7000B', b: 'rgba(231,0,11,0.25)' }
    : r === 'admin'
      ? { bg: 'rgba(157,0,16,0.12)', c: '#9D0010', b: 'rgba(157,0,16,0.25)' }
      : { bg: 'rgba(255,255,255,0.04)', c: 'var(--text-muted)', b: 'var(--border)' }
...
function exportCSV(rows) {
  ...
  const lines = [
    cols.join(','),
    ...rows.map((u) => cols.map((c) => JSON.stringify(u[c] ?? '')).join(',')),
  ]
...
function exportAuditLogsCSV(logs) {
  ...
  const lines = [
    cols.join(','),
    ...logs.map((l) => cols.map((c) => JSON.stringify(l[c] ?? '')).join(',')),
  ]
  const a = document.createElement('a')
  a.href = URL.createObjectURL(new Blob([lines.join('\n')], { type: 'text/csv' }))
...
export default function Admin() {
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      ...
      setRecentUsers(r.data?.recent_users || [])
    } catch {}
  ...
  const loadAdoption = useCallback(async () => {
    ...
    try {
      const r = await api.get('/admin/feature-adoption')
      setAdoption(r.data || null)
    } catch {}
  ...
  const loadChangelog = useCallback(async () => {
    setClLoading(true)
    try {
      const r = await api.get('/admin/changelog')
      setChangelog(r.data || [])
    } catch {}
    setClLoading(false)
  }, [])
  ...
  const loadGrowth = useCallback(async () => {
    ...
    try {
      const r = await api.get('/admin/growth')
      setGrowth(r.data || null)
    } catch {}
  ...
  const loadEngagement = useCallback(async () => {
    setEngLoading(true)
    try {
      const r = await api.get('/admin/engagement')
      setEngagement(r.data || null)
    } catch {}
    setEngLoading(false)
  }, [])
  ...
  const loadRetention = useCallback(async () => {
    ...
    try {
      const r = await api.get('/admin/retention')
  ...
  const loadAudit = useCallback(async (action, page) => {
    ...
    try {
      ...
      const r = await api.get(`/admin/audit-logs?${p}`)
  ...
  useEffect(() => {
    ...
    if (tab === 'changelog') {
      loadChangelog()
      loadedTabs.current.add('changelog')
    }
    if (tab === 'growth') {
      loadGrowth()
    ...
    if (tab === 'engagement') {
      loadEngagement()
      loadedTabs.current.add('engagement')
    }
    if (tab === 'retention') {
      loadRetention()
      loadedTabs.current.add('retention')
    }
    if (tab === 'audit') loadAudit(auditAction, auditPage)
    if (tab === 'flags') {
      loadFlags()
      loadedTabs.current.add('flags')
    }
    if (tab === 'team') {
      loadTeam()
  ...
  useEffect(() => {
    if (!dCmd || dCmd.length < 2) {
      setCmdResults(null)
      return
    }
    setCmdSearching(true)
    api
      .get(`/admin/search?q=${encodeURIComponent(dCmd)}`)
      .then((r) => {
        setCmdResults(r.data || {})
        setCmdIndex(0)
      })
      .catch(() => {})
  ...
  async function openUserSheet(u) {
    ...
    try {
      const [detail, login] = await Promise.all([
        api.get(`/admin/user/${u.id}`),
        api.get(`/admin/login-history/${u.id}`),
      ])
  ...
  async function handleToggleFlag(name, current) {
    ...
    try {
      ...
      setFlagMsg({ ok: false, text: e?.response?.data?.detail || 'Failed' })
    }
  ...
  async function handleBulkFlags(cat, enabled) {
    ...
    for (const f of targets) {
      try {
        await updateFeatureFlag(f.name, enabled)
      } catch {}
    }
    setFeatureFlags((prev) => prev.map((f) => (f.category === cat ? { ...f, enabled } : f)))
    setFlagMsg({
      ok: true,
      text: `${CAT_LABELS[cat] || cat}: all ${enabled ? 'enabled' : 'disabled'}`,
    })
    setTimeout(() => setFlagMsg(null), 4000)
  }
    ...
    try {
      if (annEditing) {
        ...
        setAnnouncements((prev) =>
          prev.map((a) => (a.id === annEditing ? { ...a, ...annForm } : a))
        )
      } else {
  ...
  async function handleClSave(e) {
    ...
    try {
      if (clEditing) {
        await api.patch(`/admin/changelog/${clEditing}`, clForm)
        setChangelog((prev) => prev.map((c) => (c.id === clEditing ? { ...c, ...clForm } : c)))
      } else {
        const r = await api.post('/admin/changelog', clForm)
        setChangelog((prev) => [r.data, ...prev])
      }
      setClForm({ title: '', body: '', version: '', published: false })
      setClEditing(null)
    } catch {}
    setClSaving(false)
  }
  ...
  async function handleClToggle(id, current) {
    try {
      await api.patch(`/admin/changelog/${id}`, { published: !current })
      setChangelog((prev) => prev.map((c) => (c.id === id ? { ...c, published: !current } : c)))
    } catch {}
  }
  ...
  async function handleDiscordSend(e) {
    ...
    setDiscordFb(null)
    try {
      await api.post('/admin/discord', { channel: discordCh, message: discordMsg.trim() })
      setDiscordFb({ ok: true, text: `Sent to #${discordCh}` })
      setDiscordMsg('')
    } catch (err) {
      setDiscordFb({ ok: false, text: err?.response?.data?.detail || 'Failed.' })
    }
  ...
  async function handleGithubInvite(e) {
    ...
    try {
      ...
      setGhFb({
        ok: true,
        text:
          status === 'invited' ? `Invite sent to ${username}` : `${username} already has access.`,
      })
      setGhLog((prev) =>
        [
          {
            ...
            ok: true,
            label: status === 'invited' ? 'Invited' : 'Already member',
          },
          ...prev,
        ].slice(0, 10)
      )
      ...
      setGhFb({ ok: false, text })
      setGhLog((prev) =>
        [
          { username, time: new Date().toLocaleTimeString(), ok: false, label: 'Failed' },
          ...prev,
        ].slice(0, 10)
      )
    }
  ...
  async function handleBlast(e) {
    ...
    setBlastResult(null)
    try {
      const r = await api.post('/admin/email-blast', blast)
      setBlastResult(r.data)
      setBlast((b) => ({ ...b, subject: '', body: '' }))
    } catch (err) {
      setBlastResult({ error: err?.response?.data?.detail || 'Failed to send.' })
    }
    setBlasting(false)
  }
  ...
  function handleCmdKeyDown(e) {
    const allResults = [
      ...(cmdResults?.users || []).map((u) => ({ type: 'user', item: u })),
      ...(cmdResults?.companies || []).map((c) => ({ type: 'company', item: c })),
      ...(cmdResults?.leads || []).map((l) => ({ type: 'lead', item: l })),
    ]
    const total = allResults.length + TABS.length
    if (e.key === 'ArrowDown') {
      e.preventDefault()
      setCmdIndex((i) => (i + 1) % total)
    }
    if (e.key === 'ArrowUp') {
      e.preventDefault()
      setCmdIndex((i) => (i - 1 + total) % total)
    }
    if (e.key === 'Enter') {
      if (cmdIndex < TABS.length) {
        setTab(TABS[cmdIndex].id)
        setCmdOpen(false)
        setCmdQuery('')
      } else {
        const r = allResults[cmdIndex - TABS.length]
        if (r?.type === 'user') {
          openUserSheet(r.item)
          setCmdOpen(false)
          setCmdQuery('')
        }
      }
          ...
          >
            {adoption.features.map((f) => (
              ...
              >
                ...
                <span style={{ ...s.sub, marginTop: 4, display: 'block' }}>
                  {f.users_adopted} of {adoption.total_users} users · {f.total} total records
                </span>
              </div>
            ))}
          </div>
      ...
      <div>
        ...
        >
          ...
          <button
            onClick={() =>
              allUsers.length &&
              exportCSV(allUsers.filter((u) => (selected.size ? selected.has(u.id) : true)))
            }
            style={s.ghostBtn}
        ...
        {usersLoading ? (
          ...
          <>
            ...
            >
              ...
              >
                {usersTotal} users · page {usersPage + 1}
              </span>
            </div>
            <div style={s.list}>
              {allUsers.map((u) => (
                <div
                  ...
                  style={{
                    ...
                    borderColor: u.suspended ? 'rgba(224,112,112,0.2)' : 'var(--border)',
                  }}
                >
                  <input
                    type="checkbox"
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        <p style={{ ...s.name, color: u.suspended ? '#e07070' : 'var(--text)' }}>
                          {u.full_name || u.email}
                        </p>
                        {u.admin_label && (
                          <span
                    ...
                    {u.id !== profile?.id && (
                      <>
                        <button
                          ...
                          style={{
                            ...s.actionBtn,
                            color: u.suspended ? '#4a7c59' : '#e07070',
        ...
        {totalPages > 1 && (
          <div style={{ display: 'flex', gap: 8, marginTop: 16, alignItems: 'center' }}>
            ...
            <span style={s.sub}>
              {usersPage + 1} / {totalPages} · {usersTotal} total
            </span>
            <button
        ...
        {selected.size > 0 && (
          ...
          >
            ...
            <button
              ...
              style={{ ...s.actionBtn, color: '#f59e0b', borderColor: 'rgba(245,158,11,0.3)' }}
            >
              Suspend all
            </button>
            <button
              onClick={() => handleBulkAction('unsuspend')}
              style={{ ...s.actionBtn, color: '#4a7c59', borderColor: 'rgba(74,124,89,0.3)' }}
            >
              Unsuspend all
            </button>
            <button
              onClick={() => handleBulkAction('admin')}
              style={{ ...s.actionBtn, color: '#9D0010', borderColor: 'rgba(157,0,16,0.3)' }}
            >
              Make admin
            </button>
            <button onClick={() => handleBulkAction('member')} style={s.actionBtn}>
              Make member
            </button>
            <button
              onClick={() => exportCSV(allUsers.filter((u) => selected.has(u.id)))}
              style={s.actionBtn}
            >
              Export CSV
            </button>
            ...
            <button
              ...
              style={{ ...s.actionBtn, color: '#e07070' }}
            >
              ✕ Clear
            </button>
          </div>
  ...
  function renderEngagement() {
    ...
    return (
      <div>
        ...
        >
          {[
            ['Top by companies', top_by_companies, 'companies'],
            ['Top by leads', top_by_leads, 'leads'],
          ].map(([title, list, key]) => (
            <div key={key}>
              ...
              {list.length === 0 ? (
                ...
                <div style={s.list}>
                  {list.map((u, i) => (
                    <div
                      key={u.id}
                      style={{ ...s.row, gap: 10, padding: '10px 14px', cursor: 'pointer' }}
                      onClick={() => openUserSheet(u)}
                    >
    ...
    return (
      <div>
        <p style={{ ...s.sub, marginBottom: 20 }}>
          8-week cohort retention — percentage of each sign-up cohort that was active in the last 7
          days.
        </p>
        <div style={{ overflowX: 'auto' }}>
          ...
          >
            ...
            <tbody>
              {cohorts.map((c, i) => {
                const pctColor =
                  c.retention_pct >= 50 ? '#059669' : c.retention_pct >= 25 ? '#f59e0b' : '#e07070'
                return (
                  <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                    <td style={{ padding: '12px 14px', color: 'var(--text)', fontWeight: 600 }}>
                      Wk {c.week}
                    </td>
                    ...
                    <td style={{ padding: '12px 14px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <span style={{ fontWeight: 700, color: pctColor, minWidth: 40 }}>
                          {c.retention_pct}%
                        </span>
                        <div
                          style={{
                        ...
                        >
                          <div
                            style={{
                              height: '100%',
                              width: `${(c.retention_pct / maxPct) * 100}%`,
                              background: pctColor,
                              borderRadius: 3,
      ...
      <>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <p style={s.sub}>
              <strong style={{ color: 'var(--text)' }}>{enabledCount}</strong> of{' '}
              {featureFlags.length} flags enabled · admins bypass all flags · propagates within 60 s
            </p>
            <button
              onClick={() => {
                loadedTabs.current.delete('flags')
                loadFlags()
              }}
              style={s.ghostBtn}
            >
          ...
          {flagMsg && (
            <span
              style={{
                fontFamily: 'var(--font-mono)',
                fontSize: 11,
                color: flagMsg.ok ? '#4a7c59' : '#e07070',
              }}
            >
              {flagMsg.ok ? '✓' : '✗'} {flagMsg.text}
            </span>
          )}
        </div>
        {categories.map((cat) => {
          ...
          return (
            <div key={cat} style={{ marginBottom: 28 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                ...
                >
                  {catEnabled}/{catFlags.length}
                </span>
                <div style={{ display: 'flex', gap: 4, marginLeft: 'auto' }}>
                  <button
                    onClick={() => handleBulkFlags(cat, true)}
                    style={{ ...s.actionBtn, fontSize: 9 }}
                  >
                    Enable all
                  </button>
                  <button
                    onClick={() => handleBulkFlags(cat, false)}
                    style={{
                      ...s.actionBtn,
                      fontSize: 9,
                      color: '#e07070',
                ...
                {catFlags.map((flag) => (
                  <div
                    ...
                    style={{
                      ...
                      padding: '12px 16px',
                      background: 'var(--surface)',
                      border: `1px solid ${flag.enabled ? 'rgba(5,150,105,0.22)' : 'var(--border)'}`,
                      borderRadius: 8,
                    }}
                  >
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p
                        style={{
                          ...
                          fontSize: 12,
                          fontWeight: 600,
                          color: flag.enabled ? 'var(--text)' : 'var(--text-muted)',
                          margin: 0,
                        }}
                      >
                        {flag.label}
                      ...
                      {flag.description && (
                        <p style={{ ...s.sub, margin: '3px 0 0', lineHeight: 1.4 }}>
                          {flag.description}
                        </p>
                      )}
                    </div>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {['info', 'warning', 'success', 'error'].map((k) => (
                <button
                  key={k}
                  type="button"
                  onClick={() => setAnnForm((f) => ({ ...f, kind: k }))}
                  style={{
                    padding: '6px 14px',
                    borderRadius: 6,
                    border: `1.5px solid ${annForm.kind === k ? KIND_COLOR[k] : 'var(--border)'}`,
                    background: annForm.kind === k ? `${KIND_COLOR[k]}18` : 'transparent',
                    color: annForm.kind === k ? KIND_COLOR[k] : 'var(--text-muted)',
                    fontFamily: 'var(--font-mono)',
                    fontSize: 11,
                    cursor: 'pointer',
                  }}
            ...
            {announcements.map((a) => (
              <div
                ...
                style={{
                  padding: '16px 20px',
                  background: 'var(--surface)',
                  border: `1px solid ${a.active ? KIND_COLOR[a.kind] + '35' : 'var(--border)'}`,
                  borderRadius: 10,
              ...
              >
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <p style={{ ...s.sub, lineHeight: 1.5 }}>{a.body}</p>
                  </div>
                  <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
                    <button onClick={() => handleAnnToggle(a.id, a.active)} style={s.actionBtn}>
                      {a.active ? 'Hide' : 'Show'}
                    </button>
                    <button
                      onClick={() => {
                        setAnnEditing(a.id)
                        setAnnForm({ title: a.title, body: a.body, kind: a.kind })
                      }}
                      style={s.actionBtn}
                    >
                      Edit
          ...
          >
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 8 }}>
              <input
                ...
                onChange={(e) => setClForm((f) => ({ ...f, version: e.target.value }))}
                style={s.input}
              />
              <input
                type="text"
            ...
            <textarea
              placeholder="Details, bug fixes, improvements…"
              rows={4}
              value={clForm.body}
              onChange={(e) => setClForm((f) => ({ ...f, body: e.target.value }))}
              style={{ ...s.input, resize: 'vertical', lineHeight: 1.6 }}
            />
            <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                <Toggle
                  on={clForm.published}
                  loading={false}
                  onToggle={() => setClForm((f) => ({ ...f, published: !f.published }))}
                />
                <span
                  style={{
                    fontFamily: 'var(--font-mono)',
                    fontSize: 11,
                    color: clForm.published ? 'var(--text)' : 'var(--text-muted)',
                  }}
                >
                  {clForm.published ? 'Published (visible to users)' : 'Draft'}
                </span>
              </label>
              <button type="submit" disabled={clSaving} style={s.primaryBtn}>
                {clSaving ? 'Saving…' : clEditing ? 'Update' : 'Create'}
              </button>
              {clEditing && (
                <button
                  type="button"
            ...
            {changelog.map((c) => (
              <div
                ...
                style={{
                  padding: '16px 20px',
                  background: 'var(--surface)',
                  border: `1px solid ${c.published ? 'rgba(5,150,105,0.22)' : 'var(--border)'}`,
                  borderRadius: 10,
              ...
              >
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                      ...
                      <span
                        style={{
                          ...
                          color: c.published ? '#059669' : 'var(--text-muted)',
                          fontFamily: 'var(--font-mono)',
                          fontSize: 8,
                          fontWeight: 700,
                        }}
                  ...
                  <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
                    <button onClick={() => handleClToggle(c.id, c.published)} style={s.actionBtn}>
      ...
      <div>
        ...
        >
          ...
          <button
            onClick={() => {
              setLiveCount(0)
              loadAudit(auditAction, auditPage)
            }}
            style={s.ghostBtn}
          >
            ↻ Refresh
          ...
          <button
            ...
            style={{ ...s.ghostBtn, opacity: auditLogs.length === 0 ? 0.4 : 1 }}
          >
            ↓ Export CSV
          </button>
          <span
              ...
              <div key={log.id} style={{ ...s.row, alignItems: 'flex-start' }}>
                <span
                  style={{
                    padding: '3px 8px',
                    borderRadius: 4,
                    background: `${ACT_COLOR[log.action] || 'rgba(255,255,255,0.1)'}18`,
                    color: ACT_COLOR[log.action] || 'var(--text-muted)',
                    fontFamily: 'var(--font-mono)',
                    fontSize: 9,
                    fontWeight: 700,
                    whiteSpace: 'nowrap',
      ...
      <div>
        <div style={{ ...s.card, marginBottom: 24 }}>
          ...
          <form onSubmit={handleInvite} style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            <button type="submit" disabled={inviting} style={s.primaryBtn}>
                ...
                {m.id !== profile?.id ? (
                  removeConfirm === m.id ? (
                    <>
                      ...
                      <button onClick={() => setRemoveConfirm(null)} style={s.actionBtn}>
                        Cancel
                      </button>
                    </>
                  ) : (
              ...
              {invites.map((inv) => (
                <div key={inv.id} style={s.row}>
                  <div style={{ flex: 1 }}>
                    ...
                    <p style={s.sub}>
                      {inv.role} · sent {fmtDate(inv.created_at)}
                    </p>
                  </div>
                  <button onClick={() => handleRevokeInvite(inv.id)} style={s.actionBtn}>
                    Revoke
                  </button>
                </div>
              ))}
        ...
        <div style={s.card}>
          ...
          >
            <div style={{ display: 'flex', gap: 8 }}>
              {[
                ...
                <button
                  key={val}
                  type="button"
                  onClick={() => setBlast((b) => ({ ...b, segment: val }))}
                  style={{
                    padding: '7px 14px',
                    borderRadius: 6,
                    border: `1.5px solid ${blast.segment === val ? '#E7000B' : 'var(--border)'}`,
                    background: blast.segment === val ? 'rgba(231,0,11,0.1)' : 'transparent',
                    color: blast.segment === val ? '#E7000B' : 'var(--text-muted)',
                    fontFamily: 'var(--font-mono)',
                    fontSize: 11,
                    fontWeight: 600,
                    cursor: 'pointer',
            ...
            <textarea
              ...
              rows={5}
              value={blast.body}
              onChange={(e) => setBlast((b) => ({ ...b, body: e.target.value }))}
              style={{ ...s.input, resize: 'vertical', lineHeight: 1.6 }}
            />
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <button
                type="submit"
                disabled={blasting || !blast.subject.trim() || !blast.body.trim()}
                style={{
                  ...s.primaryBtn,
                  opacity: !blast.subject.trim() || !blast.body.trim() ? 0.4 : 1,
                }}
              >
              ...
              {blastResult && (
                <span
                  style={{
                    fontFamily: 'var(--font-mono)',
                    fontSize: 11,
                    color: blastResult.error ? '#e07070' : '#4a7c59',
                  }}
                >
                  {blastResult.error
                    ? `✗ ${blastResult.error}`
                    : `✓ Sent to ${blastResult.sent} · ${blastResult.errors} errors`}
                </span>
              )}
            </div>
          </form>
          ...
          <>
              ...
              >
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {DISCORD_CH.map((ch) => (
                    <button
                      key={ch.id}
                      type="button"
                      onClick={() => setDiscordCh(ch.id)}
                      style={{
                        padding: '7px 14px',
                        borderRadius: 6,
                        border: `1.5px solid ${discordCh === ch.id ? ch.color : 'var(--border)'}`,
                        background: discordCh === ch.id ? `${ch.color}18` : 'transparent',
                        color: discordCh === ch.id ? ch.color : 'var(--text-muted)',
                        fontFamily: 'var(--font-mono)',
                        fontSize: 11,
                        fontWeight: 600,
                        cursor: 'pointer',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <button
                    ...
                    style={{ ...s.primaryBtn, opacity: !discordMsg.trim() ? 0.4 : 1 }}
                  >
                  ...
                  {discordFb && (
                    <span
                      style={{
                        fontFamily: 'var(--font-mono)',
                        fontSize: 11,
                        color: discordFb.ok ? '#4a7c59' : '#e07070',
                      }}
                    >
                      {discordFb.ok ? '✓ ' : '✗ '}
                      {discordFb.text}
            ...
            <div style={s.card}>
              ...
              <p style={{ ...s.sub, marginBottom: 14 }}>
                Team member posts their GitHub username → enter it here → they get an invite by
                email.
              </p>
              <form
                onSubmit={handleGithubInvite}
              ...
              >
                ...
                <button
                  ...
                  style={{ ...s.primaryBtn, opacity: !ghUsername.trim() ? 0.4 : 1 }}
                >
    ...
    return (
      <div>
        ...
        >
          ...
          <button onClick={loadHealth} disabled={healthLoading} style={s.ghostBtn}>
        ...
        {healthLoading ? (
          ...
          <Empty text="Click 'Run health check' to see status." />
        ) : (
          <div
            style={{
              display: 'grid',
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <div
        style={{
          padding: 'clamp(28px,4vw,64px) clamp(20px,4vw,48px) 0',
          borderBottom: '1px solid var(--border)',
        }}
      >
        <div
      ...
      <div
        style={{
          maxWidth: 1000,
          margin: '0 auto',
          padding: 'clamp(24px,3vw,40px) clamp(20px,4vw,48px) 80px',
        }}
      >
        {(tabContent[tab] || (() => null))()}
      </div>
          ...
          >
            ...
            >
              ...
              <input
                ...
                placeholder="Search users, companies, leads or jump to tab…"
                style={{
                  flex: 1,
                  background: 'none',
                  border: 'none',
                  ...
                  {TABS.map((t, i) => (
                    <button
                      ...
                      style={{
                        ...
                        width: '100%',
                        padding: '10px 18px',
                        background: cmdIndex === i ? 'rgba(255,255,255,0.06)' : 'none',
                        border: 'none',
                        cursor: 'pointer',
                        textAlign: 'left',
                      }}
              ...
              {cmdQuery && cmdResults && (
                <>
                      ...
                      {cmdResults.users.map((u, i) => (
                        <button
                          ...
                          style={{
                            ...
                            padding: '10px 18px',
                            background:
                              cmdIndex === TABS.length + i ? 'rgba(255,255,255,0.06)' : 'none',
                            border: 'none',
                            cursor: 'pointer',
                          }}
                        >
                  ...
                  {!cmdResults.users?.length &&
                    !cmdResults.companies?.length &&
                    !cmdResults.leads?.length && (
                      <p
                        style={{
                          fontFamily: 'var(--font-mono)',
                          fontSize: 12,
  ...
  return (
    <button
      ...
      style={{
        ...
        position: 'relative',
        background: on ? '#059669' : 'var(--border)',
      ...
      {(sub || delta != null) && (
        <p
          style={{
            fontFamily: 'var(--font-mono)',
            fontSize: 10,
            color: delta > 0 ? '#4a7c59' : 'var(--text-muted)',
            marginTop: 6,
          }}
        >
...
function LineChart({ days, series }) {
  ...
  const pts = (data) =>
    data
      .map(
        (v, i) =>
          `${(PL + (i / Math.max(data.length - 1, 1)) * cW).toFixed(1)},${(H - PB - (v / maxVal) * (cH - 4) - 4).toFixed(1)}`
      )
      .join(' ')
  const labelIdxs = days
    .map((_, i) => i)
    .filter((i) => i % Math.ceil(days.length / 6) === 0 || i === days.length - 1)
  return (
    <div>
      ...
      <div style={{ overflowX: 'auto', borderRadius: 8 }}>
        ...
        >
          ...
          {labelIdxs.map((i) => (
            <text
              key={i}
              x={PL + (i / Math.max(days.length - 1, 1)) * cW}
              y={H - 2}
              textAnchor="middle"
              fontFamily="monospace"
              fontSize="8"
            ...
            s.data.map(
              (v, i) =>
                v > 0 && (
                  <circle
                    key={i}
                    cx={PL + (i / Math.max(s.data.length - 1, 1)) * cW}
                    cy={H - PB - (v / maxVal) * (cH - 4) - 4}
                    r="2.5"
                    fill={s.color}
                    opacity="0.8"
                  />
...
function UserSheet({ user, detail, detailLoading, onClose, onUpdate, currentUserId }) {
  ...
  async function saveNotes() {
    setSaving(true)
    try {
      await onUpdate({ admin_notes: notes, admin_label: label || null })
      setSaved(true)
        ...
        >
              ...
              [
                ...
                <span
                  style={{
                    fontFamily: 'var(--font-mono)',
                    fontSize: 11,
                    color: user.suspended ? '#e07070' : '#4a7c59',
                  }}
                >
                  {user.suspended ? 'Suspended' : 'Active'}
                </span>,
              ],
              ['Companies', counts.companies ?? user.companies ?? 0],
              ['Leads', counts.leads ?? user.leads ?? 0],
              ['Sequences', counts.sequences ?? 0],
              ['Tasks', counts.tasks ?? 0],
              ['Joined', fmtDate(user.created_at || user.joined)],
              ['Last login', auth.last_sign_in ? fmtDate(auth.last_sign_in) : '—'],
            ].map(([lbl, val]) => (
              <div
                key={lbl}
                style={{
            ...
            >
              <span
                style={{
                  fontFamily: 'var(--font-mono)',
                  fontSize: 10,
                  color: auth.email_confirmed ? '#4a7c59' : '#f59e0b',
                }}
              >
                {auth.email_confirmed ? '✓ Email confirmed' : '⚠ Email not confirmed'}
              </span>
            </div>
          )}
              ...
              {LABELS.map((l) => (
                <button
                  ...
                  style={{
                    padding: '6px 12px',
                    borderRadius: 6,
                    border: `1.5px solid ${label === l ? '#8b5cf6' : 'var(--border)'}`,
                    background: label === l ? 'rgba(139,92,246,0.12)' : 'transparent',
                    color: label === l ? '#8b5cf6' : 'var(--text-muted)',
                    fontFamily: 'var(--font-mono)',
                    fontSize: 10,
                    cursor: 'pointer',
                  }}
          ...
          <div>
            ...
            <button
              onClick={saveNotes}
              disabled={saving}
              style={{ ...s.primaryBtn, marginTop: 8, fontSize: 11, padding: '8px 16px' }}
            >
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <button
                  ...
                  style={{
                    padding: '10px 16px',
                    background: 'var(--surface)',
                    border: `1px solid ${user.suspended ? 'rgba(74,124,89,0.3)' : 'rgba(224,112,112,0.25)'}`,
                    borderRadius: 7,
                    fontFamily: 'var(--font-mono)',
                    fontSize: 12,
                    color: user.suspended ? '#4a7c59' : '#e07070',
                    cursor: 'pointer',
                    textAlign: 'left',
                  }}
                >
