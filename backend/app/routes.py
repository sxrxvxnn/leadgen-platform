...
import datetime as _li_dt

_LI_CLIENT_ID     = os.environ.get("LINKEDIN_CLIENT_ID", "253300048")
...
def _li_make_state(user_id: str) -> str:
    ts  = str(int(_li_time.time()))
    msg = f"{user_id}|{ts}"
    sig = _hashlib.sha256(f"{_LI_STATE_SECRET}:{msg}".encode()).hexdigest()[:16]
    raw = f"{msg}|{sig}"
    return base64.urlsafe_b64encode(raw.encode()).decode().rstrip("=")


def _li_parse_state(state: str) -> str:
    try:
        padded = state + "=" * (-len(state) % 4)
        raw    = base64.urlsafe_b64decode(padded).decode()
        body, sig = raw.rsplit("|", 1)
        user_id, ts = body.split("|", 1)
        expected = _hashlib.sha256(f"{_LI_STATE_SECRET}:{body}".encode()).hexdigest()[:16]
        if sig != expected:
            raise ValueError("bad sig")
        if int(_li_time.time()) - int(ts) > 600:
...
async def linkedin_auth(authorization: str = Header(...)):
    user_id = get_user_id(authorization)
    state   = _li_make_state(user_id)
    params  = {
        "response_type": "code",
    ...
    url = "https://www.linkedin.com/oauth/v2/authorization?" + _urllib_parse.urlencode(params)
    return {"auth_url": url}
...
async def linkedin_callback(
    ...
    dashboard_url = os.environ.get("FRONTEND_URL", "https://sonarleads.vercel.app")

    if error or not code or not state:
        return RedirectResponse(f"{dashboard_url}/settings?li_error={error or 'missing_code'}")

    user_id = _li_parse_state(state)
    ...
    try:
        ...
        return RedirectResponse(f"{dashboard_url}/settings?li_error=token_exchange_failed")

    if "access_token" not in token_data:
        return RedirectResponse(f"{dashboard_url}/settings?li_error=no_token")

    access_token  = token_data["access_token"]
    refresh_token = token_data.get("refresh_token")
    expires_in    = token_data.get("expires_in", 5183944)
    token_expires = (_li_dt.datetime.utcnow() + _li_dt.timedelta(seconds=expires_in)).isoformat()
    ...
    supabase.table("linkedin_connections").upsert({
        "user_id":              user_id,
    ...
    return RedirectResponse(f"{dashboard_url}/settings?li_connected=1")


@router.get("/linkedin/status")
async def linkedin_status(authorization: str = Header(...)):
    user_id = get_user_id(authorization)
    res = supabase.table("linkedin_connections") \
        .select("linkedin_person_urn, selected_account_urn, last_synced_at, created_at") \
    ...
    return {
        "connected":             True,
        "person_urn":            conn.get("linkedin_person_urn"),
        "selected_account_urn":  conn.get("selected_account_urn"),
        "last_synced_at":        conn.get("last_synced_at"),
        "connected_since":       conn.get("created_at"),
    }


@router.delete("/linkedin/disconnect")
async def linkedin_disconnect(authorization: str = Header(...)):
    user_id = get_user_id(authorization)
    supabase.table("linkedin_connections").delete().eq("user_id", user_id).execute()
    return {"ok": True}


@router.get("/linkedin/ad-accounts")
async def linkedin_ad_accounts(authorization: str = Header(...)):
    user_id = get_user_id(authorization)
    conn = supabase.table("linkedin_connections").select("access_token") \
        .eq("user_id", user_id).maybe_single().execute()
    if not conn.data:
    ...
    try:
        res = requests.get(
            ...
            params={"q": "search", "search.type.values[0]": "BUSINESS", "count": 50},
        ...
        accounts = [
            {
                "urn":      a.get("id"),
                "name":     a.get("name", "Unknown Account"),
                "status":   a.get("status"),
...
@router.post("/linkedin/select-account")
async def linkedin_select_account(payload: dict, authorization: str = Header(...)):
    user_id     = get_user_id(authorization)
    account_urn = payload.get("account_urn")
    if not account_urn:
        raise HTTPException(status_code=400, detail="account_urn required")
    supabase.table("linkedin_connections") \
        .update({"selected_account_urn": account_urn}) \
        .eq("user_id", user_id).execute()
    return {"ok": True}


@router.post("/linkedin/sync-now")
async def linkedin_sync_now(authorization: str = Header(...)):
    user_id = get_user_id(authorization)
    conn = supabase.table("linkedin_connections").select("*") \
        .eq("user_id", user_id).maybe_single().execute()
    if not conn.data:
        raise HTTPException(status_code=404, detail="LinkedIn not connected")
    ...
    account_urn  = connection.get("selected_account_urn")
    if not account_urn:
    ...
    try:
        forms_res = requests.get(
            "https://api.linkedin.com/v2/leadGenerationForms",
            params={"q": "account", "account": account_urn, "count": 100},
            headers={"Authorization": f"Bearer {token}", "LinkedIn-Version": "202401"},
            timeout=15,
        )
        forms = forms_res.json().get("elements", [])
    except Exception as e:
        raise HTTPException(status_code=502, detail=f"Failed to fetch forms: {e}")

    synced = 0
    for form in forms:
        form_id   = form.get("id")
        form_name = form.get("name", "LinkedIn Form")
        if not form_id:
            continue

        try:
            resp_res = requests.get(
                "https://api.linkedin.com/v2/adLeadGenerationFormResponses",
                params={"q": "byLeadGenerationForm", "leadGenerationFormId": form_id, "count": 100},
                headers={"Authorization": f"Bearer {token}", "LinkedIn-Version": "202401"},
                timeout=15,
            )
            responses = resp_res.json().get("elements", [])
        except Exception:
            continue

        for resp in responses:
            answers = resp.get("formResponse", {}).get("answers", [])
            lead: dict = {"source": "linkedin_lead_gen", "status": "new"}
            for ans in answers:
                qt, val = ans.get("questionType", ""), ans.get("answer", "")
                if qt == "FIRST_NAME":                   lead["first_name"] = val
                elif qt == "LAST_NAME":                  lead["last_name"]  = val
                elif qt == "EMAIL":                      lead["email"]      = val
                elif qt in ("COMPANY", "WORK_COMPANY"):  lead["company"]    = val
                elif qt in ("TITLE", "WORK_TITLE"):      lead["title"]      = val
                elif qt == "PHONE":                      lead["phone"]      = val
                elif qt == "LINKEDIN_MEMBER_URN":
                    lead["linkedin_url"] = f"https://www.linkedin.com/in/{val}"

            if not lead.get("email") and not lead.get("first_name"):
                continue

            # Deduplicate by email
            if lead.get("email"):
                existing = supabase.table("leads").select("id") \
                    .eq("user_id", user_id).eq("email", lead["email"]).execute()
                if existing.data:
                    continue

            name = " ".join(
                p for p in [lead.pop("first_name", ""), lead.pop("last_name", "")] if p
            ).strip() or "LinkedIn Lead"

            supabase.table("leads").insert({
                **lead,
                "name":    name,
                "user_id": user_id,
                "notes":   f"Synced from LinkedIn Lead Gen Form: {form_name}",
            }).execute()
            synced += 1

    supabase.table("linkedin_connections") \
        .update({"last_synced_at": _li_dt.datetime.utcnow().isoformat()}) \
        .eq("user_id", user_id).execute()

    return {"synced": synced, "forms_checked": len(forms)}


# Public webhook — LinkedIn calls this when a new lead submits a form
@router.post("/linkedin/lead-sync")
async def linkedin_lead_sync_webhook(request: Request):
    try:
    ...
    for item in items:
        answers = item.get("formResponse", {}).get("answers", [])
        if not answers:
            continue
        ...
        connections = supabase.table("linkedin_connections").select("user_id").execute().data or []
        for conn in connections:
            uid = conn["user_id"]
            if lead.get("email"):
                existing = supabase.table("leads").select("id") \
                    .eq("user_id", uid).eq("email", lead["email"]).execute()
                if existing.data:
                    continue
            supabase.table("leads").insert({
                **lead,
                "name":    name,
                "user_id": uid,
                "notes":   "Auto-synced from LinkedIn Lead Gen Form webhook",
            }).execute()
